...
describe('CKFinderCommand', () => {
  ...
  describe('execute()', () => {
    ...
    it('should insert single chosen image', () => {
      ...
      expect(getModelData(model)).to.equal(`[<image src="${url}"></image>]<paragraph>foo</paragraph>`);
    });
    ...
    it('should insert link if chosen file is not an image', () => {
      ...
      expect(getModelData(model)).to.equal(`<paragraph>f[<$text linkHref="${url}">o</$text>]o</paragraph>`);
    });
    ...
    it('should use CKFinder Proxy for privately hosted files', () => {
      ...
      expect(getModelData(model)).to.equal(`[<image src="${proxyUrl}"></image>]<paragraph>foo</paragraph>`);
    });
