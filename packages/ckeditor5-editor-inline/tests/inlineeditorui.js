...
describe('InlineEditorUI', () => {
  ...
  beforeEach(() => {
    return VirtualInlineTestEditor.create('foo', {
      toolbar: ['foo', 'bar'],
    }).then(newEditor => {
      editor = newEditor;
  ...
  describe('init()', () => {
    ...
    describe('placeholder', () => {
      ...
      it('sets placeholder from the "placeholder" attribute of a passed <textarea>', () => {
        ...
        return VirtualInlineTestEditor.create(element, {
          extraPlugins: [Paragraph],
        }).then(newEditor => {
          const firstChild = newEditor.editing.view.document.getRoot().getChild(0);
    ...
    it('initializes keyboard navigation between view#toolbar and view#editable', () => {
      return VirtualInlineTestEditor.create('').then(editor => {
        const ui = editor.ui;
        const view = ui.view;
  ...
  describe('destroy()', () => {
    it('detaches the DOM root then destroys the UI view', () => {
      return VirtualInlineTestEditor.create('').then(newEditor => {
    ...
    it('restores the editor element back to its original state', () => {
      ...
      return VirtualInlineTestEditor.create(domElement).then(newEditor => {
        return newEditor.destroy().then(() => {
          const attributes = {};
