...
describe('MediaEmbedEditing', () => {
    ...
    describe('configuration', () => {
      describe('#providers', () => {
        ...
        it('upcast media according to the order', () => {
          return createTestEditor({
            providers: [testProviders.A, testProviders.B, testProviders.C],
          }).then(editor => {
        ...
        describe('default value', () => {
          beforeEach(() => {
            return createTestEditor().then(newEditor => {
              editor = newEditor;
          ...
          describe('with preview', () => {
            it('upcasts the URL (dailymotion)', () => {
              testMediaUpcast(
                ['https://www.dailymotion.com/video/foo', 'www.dailymotion.com/video/foo', 'dailymotion.com/video/foo'],
          ...
          describe('preview-less', () => {
            it('upcasts the URL (instagram)', () => {
              testMediaUpcast(['https://www.instagram.com/p/foo', 'www.instagram.com/p/foo', 'instagram.com/p/foo']);
            });

            it('upcasts the URL (twitter)', () => {
              testMediaUpcast(['https://www.twitter.com/foo/bar', 'www.twitter.com/foo/bar', 'twitter.com/foo/bar']);
            });

            it('upcasts the URL (google maps)', () => {
              testMediaUpcast(['https://www.google.com/maps/foo', 'www.google.com/maps/foo', 'google.com/maps/foo']);
            });

            it('upcasts the URL (flickr)', () => {
              testMediaUpcast(['https://www.flickr.com/foo/bar', 'www.flickr.com/foo/bar', 'flickr.com/foo/bar']);
            });

            it('upcasts the URL (facebook)', () => {
              testMediaUpcast(['https://www.facebook.com/foo/bar', 'www.facebook.com/foo/bar', 'facebook.com/foo/bar']);
            });
          });
        });
      });
      ...
      describe('#extraProviders', () => {
        ...
        it('extend #providers but with the lower priority', () => {
          return createTestEditor({
            providers: [testProviders.A],
            extraProviders: [testProviders.extraA, testProviders.extraB],
          }).then(editor => {
            editor.setData('<figure class="media"><div data-oembed-url="foo.com/123"></div></figure>');
      ...
      describe('#removeProviders', () => {
        it('removes #providers', () => {
          return createTestEditor({
            providers: [testProviders.A, testProviders.B],
            removeProviders: ['A'],
          }).then(editor => {
            editor.setData(
        ...
        it('removes #extraProviders', () => {
          return createTestEditor({
            providers: [],
            extraProviders: [testProviders.A, testProviders.B],
            removeProviders: ['A'],
          }).then(editor => {
            editor.setData(
              '<figure class="media"><div data-oembed-url="foo.com/123"></div></figure>' +
        ...
        it('removes even when the name of the provider repeats', () => {
          return createTestEditor({
            providers: [testProviders.A, testProviders.A],
            extraProviders: [testProviders.A, testProviders.A, testProviders.B],
            removeProviders: ['A'],
          }).then(editor => {
            editor.setData(
              '<figure class="media"><div data-oembed-url="foo.com/123"></div></figure>' +
  ...
  describe('init()', () => {
    const providerDefinitions = [testProviders.previewLess, testProviders.allowEverything];
        ...
        describe('model to view', () => {
          it('should convert', () => {
            ...
            expect(editor.getData()).to.equal(
              '<figure class="media">' + '<oembed url="https://ckeditor.com"></oembed>' + '</figure>'
            );
          });
          ...
          it('should convert (no url)', () => {
            ...
            expect(editor.getData()).to.equal('<figure class="media">' + '<oembed></oembed>' + '</figure>');
          });
          ...
          it('should convert (preview-less media)', () => {
            ...
            expect(editor.getData()).to.equal(
              '<figure class="media">' + '<oembed url="https://preview-less"></oembed>' + '</figure>'
            );
          });
        });
        ...
        describe('view to model', () => {
          it('should convert media figure', () => {
            ...
            expect(getModelData(model, { withoutSelection: true })).to.equal(
              '<media url="https://ckeditor.com"></media>'
            );
          });
          ...
          it('should not convert in the wrong context', () => {
            ...
            expect(getModelData(model, { withoutSelection: true })).to.equal('<blockquote></blockquote>');
          });
  ...
  function testMediaUpcast(urls, expected) {
    for (const url of urls) {
      ...
      if (expected) {
        expectedRegExp = new RegExp('<figure[^>]+>' + '<div[^>]+>' + normalizeHtml(expected) + '</div>' + '</figure>');
      } else {
