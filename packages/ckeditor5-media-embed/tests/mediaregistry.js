...
describe('MediaRegistry', () => {
  describe('constructor()', () => {
    ...
    it('allows extending providers using `extraProviders` option', () => {
      const providers = [{ name: 'dailymotion', url: [] }, { name: 'youtube', url: [] }, { name: 'vimeo', url: [] }];
      const extraProviders = [{ name: 'spotify', url: [] }];
  ...
  describe('_getMedia()', () => {
    ...
    it('passes the entire match array to render function', () => {
      ...
      expect(htmlSpy.firstCall.args[0]).to.deep.equal(['youtube.com/watch?v=euqbMkM-QQk', 'euqbMkM-QQk']);
    });
  });
});
