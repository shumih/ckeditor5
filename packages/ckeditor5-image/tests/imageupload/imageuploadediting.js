...
import {
  UploadAdapterMock,
  createNativeFileMock,
  NativeFileReaderMock,
} from '@ckeditor/ckeditor5-upload/tests/_utils/mocks';
...
describe('ImageUploadEditing', () => {
  ...
  it('should insert image when is pasted', () => {
    ...
    const targetRange = model.createRange(
      model.createPositionAt(doc.getRoot(), 1),
      model.createPositionAt(doc.getRoot(), 1)
    );
  ...
  it('should insert image when is pasted on allowed position when ImageUploadCommand is disabled', () => {
    ...
    const targetRange = model.createRange(
      model.createPositionAt(doc.getRoot(), 0),
      model.createPositionAt(doc.getRoot(), 0)
    );
    const targetViewRange = editor.editing.mapper.toViewRange(targetRange);
  ...
  it("should not convert image's uploadId attribute if is consumed already", () => {
    ...
    expect(getViewData(view)).to.equal(
      '[<figure class="ck-widget image" contenteditable="false">' + '<img></img>' + '</figure>]'
    );
  });
  ...
  it('should upload image with blob src', done => {
    editor.plugins.get('Clipboard').on(
      ...
      () => {
        const id = adapterMocks[0].loader.id;
        const expected =
          `[<image src="" uploadId="${id}" uploadStatus="reading"></image>]` + '<paragraph>foo</paragraph>';
  ...
  it('should upload only images which were successfully fetched and remove failed ones', done => {
    ...
    expectData('', expectedModel, expectedFinalModel, done);

    setModelData(model, '<paragraph>[]foo</paragraph>');

    const clipboardHtml =
      `<p>bar</p><img src=${base64Sample} />` +
