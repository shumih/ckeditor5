  ...
  describe('conversion in data pipeline', () => {
    describe('model to view', () => {
      ...
      it('should not convert srcset attribute if has wrong data', () => {
        setModelData(
          model,
          '<image ' + 'src="/assets/sample.png" ' + 'alt="alt text" ' + 'srcset=\'{ "foo":"bar" }\'>' + '</image>'
        );
    ...
    describe('view to model', () => {
      it('should convert image figure', () => {
        ...
        expect(getModelData(model, { withoutSelection: true })).to.equal(
          '<image alt="alt text" src="/assets/sample.png"></image>'
        );
      });
      ...
      it('should dispatch conversion for nested elements', () => {
        ...
        editor.setData(
          '<figure class="image"><img src="/assets/sample.png" alt="alt text" /><figcaption></figcaption></figure>'
        );
      ...
      describe('should autohoist images', () => {
        ...
        it('image between non-hoisted elements', () => {
          ...
          expect(getModelData(model, { withoutSelection: true })).to.equal(
            '<div>foo</div>' + '<image alt="foo" src="foo.jpg"></image>' + '<div>bar</div>'
          );
        });
        ...
        it('images on borders of parent', () => {
          ...
          expect(getModelData(model, { withoutSelection: true })).to.equal(
            '<image alt="foo" src="foo.jpg"></image>' + '<div>foobar</div>' + '<image alt="foo" src="foo.jpg"></image>'
          );
        });
        ...
        it('images are only content of parent', () => {
          ...
          expect(getModelData(model, { withoutSelection: true })).to.equal(
            '<image alt="foo" src="foo.jpg"></image>' + '<image alt="foo" src="foo.jpg"></image>'
          );
        });
        ...
        it('deep autohoisting #2', () => {
          ...
          editor.setData(
            '<div>x</div>' + '<div><div><div><img src="foo.jpg" alt="foo" /></div></div></div>' + '<div>y</div>'
          );
  ...
  describe('conversion in editing pipeline', () => {
    describe('model to view', () => {
      ...
      it('should remove sizes and srcsset attribute when srcset attribute is removed from model', () => {
        setModelData(
          model,
          '<image src="/assets/sample.png" srcset=\'{ "data": "small.png 148w, big.png 1024w" }\'></image>'
        );
        const image = doc.getRoot().getChild(0);
