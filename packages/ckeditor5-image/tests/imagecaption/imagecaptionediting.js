...
describe('ImageCaptionEditing', () => {
  ...
  describe('data pipeline', () => {
    describe('view to model', () => {
      it('should convert figcaption inside image figure', () => {
        editor.setData(
          '<figure class="image"><img src="/assets/sample.png" /><figcaption>foo bar</figcaption></figure>'
        );

        expect(getModelData(model, { withoutSelection: true })).to.equal(
          '<image src="/assets/sample.png"><caption>foo bar</caption></image>'
        );
      });
      ...
      it('should not convert figcaption inside other elements than image', () => {
        ...
        expect(getModelData(model, { withoutSelection: true })).to.equal('<widget>foobar</widget>');
      });
    });
  ...
  describe('inserting image to the document', () => {
    it('should add caption element if image does not have it', () => {
      ...
      expect(getModelData(model)).to.equal('[<image alt="" src=""><caption></caption></image>]<paragraph></paragraph>');
  ...
  describe('editing view', () => {
    ...
    it('should not remove figcaption when selection is moved from it to other image', () => {
      setModelData(
        model,
        '<image src=""><caption>[foo bar]</caption></image><image src=""><caption></caption></image>'
      );
      const image = doc.getRoot().getChild(1);
