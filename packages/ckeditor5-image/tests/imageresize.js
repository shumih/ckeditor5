...
describe('ImageResize', () => {
  ...
  const IMAGE_SRC_FIXTURE =
    'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAGQAAAAyCAQAAAAAPLY1AAAAQklEQVR42u3PQREAAAgDoK1/' +
  ...
  describe('conversion', () => {
    ...
    it('downcasts 50% width correctly', () => {
      ...
      expect(editor.getData()).to.equal(
        `<figure class="image image_resized" style="width:50%;"><img src="${IMAGE_SRC_FIXTURE}"></figure>`
      );
    });
    ...
    it('removes style and extra class when no longer resized', () => {
      ...
      expect(editor.getData()).to.equal(`<figure class="image"><img src="${IMAGE_SRC_FIXTURE}"></figure>`);
    });
  ...
  describe('command', () => {
    ...
    it('disables the resizer if the command is disabled', () => {
      ...
      editor.commands.get('imageResize').on(
        'set:isEnabled',
        evt => {
  ...
  describe('srcset integration', () => {
    ...
    it('retains width after removing srcset', () => {
      ...
      const expectedHtml =
        '<figure class="image image_resized" style="width:80px;"><img src="/assets/sample.png"></figure>';
  ...
  function getSelectedImageResizer(editor) {
    return editor.plugins
      .get('WidgetResize')
      ._getResizerByViewElement(editor.editing.view.document.selection.getSelectedElement());
  }
