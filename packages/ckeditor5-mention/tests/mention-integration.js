...
describe('Mention feature - integration', () => {
  ...
  describe('with undo', () => {
    beforeEach(() => {
      return ClassicTestEditor.create(div, { plugins: [Paragraph, MentionEditing, UndoEditing] }).then(newEditor => {
        editor = newEditor;
        model = editor.model;
        doc = model.document;
    ...
    function testAttributePostFixer(initialData, expectedData, testCallback) {
      ...
      expect(getViewData(editor.editing.view, { withoutSelection: true })).to.equal(expectedData);

      editor.execute('undo');

      expect(editor.getData()).to.equal(initialData);
      expect(getViewData(editor.editing.view, { withoutSelection: true })).to.equal(initialData);
  ...
  describe('with clipboard', () => {
    ...
    beforeEach(() => {
      return ClassicTestEditor.create(div, {
        plugins: [Clipboard, Paragraph, BlockQuote, MentionEditing, UndoEditing],
      }).then(newEditor => {
        editor = newEditor;
        model = editor.model;
        doc = model.document;
