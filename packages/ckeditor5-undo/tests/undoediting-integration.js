...
describe('UndoEditing integration', () => {
  ...
  beforeEach(() => {
    ...
    return ClassicEditor.create(div, {
      plugins: [Paragraph, HeadingEditing, Typing, Enter, Clipboard, BoldEditing, UndoEditing],
    }).then(newEditor => {
      editor = newEditor;
  ...
  function setSelection(pathA, pathB) {
    model.change(writer => {
      writer.setSelection(
        writer.createRange(writer.createPositionFromPath(root, pathA), writer.createPositionFromPath(root, pathB))
      );
    });
  }
