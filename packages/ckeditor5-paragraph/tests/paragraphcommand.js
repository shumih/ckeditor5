  ...
  describe('execute()', () => {
    ...
    it('should not rename blocks which cannot become paragraphs (paragraph is not allowed in their parent)', () => {
      ...
      expect(getData(model)).to.equal(
        '<paragraph>a[bc</paragraph>' + '<restricted><fooBlock></fooBlock></restricted>' + '<paragraph>de]f</paragraph>'
      );
    });
    ...
    it('should not rename blocks which cannot become paragraphs (block is an object)', () => {
      ...
      setData(model, '<heading1>a[bc</heading1>' + '<image></image>' + '<heading1>de]f</heading1>');

      command.execute();

      expect(getData(model)).to.equal(
        '<paragraph>a[bc</paragraph>' + '<image></image>' + '<paragraph>de]f</paragraph>'
      );
    });
    ...
    describe('custom options', () => {
      ...
      it('should use provided selection', () => {
        ...
        const selection = model.createSelection(
          model.createRange(model.createPositionAt(secondToLastHeading, 0), model.createPositionAt(lastHeading, 1))
        );
