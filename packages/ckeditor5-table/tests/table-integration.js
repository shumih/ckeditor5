...
describe('Table feature – integration', () => {
  describe('with clipboard', () => {
    ...
    beforeEach(() => {
      return VirtualTestEditor.create({
        plugins: [Paragraph, TableEditing, ListEditing, BlockQuoteEditing, Widget, Clipboard],
      }).then(newEditor => {
        editor = newEditor;
    ...
    it('pastes td as p when pasting into the table', () => {
      ...
      expect(formatTable(getModelData(editor.model))).to.equal(formattedModelTable([['foobar[]']]));
    });
    ...
    it('pastes list into the td', () => {
      ...
      expect(formatTable(getModelData(editor.model))).to.equal(
        formattedModelTable([['<listItem listIndent="0" listType="bulleted">bar[]</listItem>']])
      );
    });
    ...
    it('pastes blockquote into the td', () => {
      ...
      expect(formatTable(getModelData(editor.model))).to.equal(
        formattedModelTable([['<blockQuote><paragraph>bar[]</paragraph></blockQuote>']])
      );
    });
  });
  ...
  describe('with undo', () => {
    ...
    beforeEach(() => {
      return VirtualTestEditor.create({ plugins: [Paragraph, TableEditing, Widget, UndoEditing] }).then(newEditor => {
        editor = newEditor;
  ...
  describe('other', () => {
    ...
    beforeEach(() => {
      return VirtualTestEditor.create({
        plugins: [Paragraph, TableEditing, ListEditing, BlockQuoteEditing, Widget, Typing],
      }).then(newEditor => {
        editor = newEditor;
      });
    });
    ...
    it('merges elements without throwing errors', () => {
      setModelData(
        editor.model,
        modelTable([['<blockQuote><paragraph>Foo</paragraph></blockQuote><paragraph>[]Bar</paragraph>']])
      );

      editor.execute('delete');

      expect(formatTable(getModelData(editor.model))).to.equal(
        formattedModelTable([['<blockQuote><paragraph>Foo[]Bar</paragraph></blockQuote>']])
      );
    });
  });
});
