...
export function viewTable(tableData, attributes = {}) {
  ...
  const thead =
    headingRows > 0
      ? `<thead>${makeRows(tableData.slice(0, headingRows), {
          cellElement: 'th',
          rowElement: 'tr',
          headingElement: 'th',
  ...
  const figureAttributes = asWidget
    ? 'class="ck-widget ck-widget_with-selection-handler table" contenteditable="false"'
    : 'class="table"';
...
function makeRows(tableData, options) {
  ...
  return tableData.reduce((previousRowsString, tableRow) => {
