...
describe('TableEditing', () => {
  ...
  describe('conversion in data pipeline', () => {
    ...
    describe('view to model', () => {
      ...
      it('should convert table with image', () => {
        ...
        expect(getModelData(model, { withoutSelection: true })).to.equal(
          '<table><tableRow><tableCell><image src="sample.png"></image></tableCell></tableRow></table>'
        );
      });

      it('should insert a paragraph when the cell content is unsupported', () => {
        editor.setData('<table><tbody><tr><td><foo></foo></td></tr></tbody></table>');
  ...
  describe('caret movement', () => {
    ...
    it('should do nothing if not tab pressed', () => {
      ...
      expect(formatTable(getModelData(model))).to.equal(formattedModelTable([['11', '12[]']]));
    });
    ...
    describe('on TAB', () => {
      ...
      it('should move to the first cell of next row if on end of a row', () => {
        setModelData(model, modelTable([['11', '12[]'], ['21', '22']]));

        editor.editing.view.document.fire('keydown', domEvtDataStub);

        expect(formatTable(getModelData(model))).to.equal(formattedModelTable([['11', '12'], ['[21]', '22']]));
      });

      it('should move to the next table cell if part of block content is selected', () => {
        setModelData(
          model,
          modelTable([['11', '<paragraph>12</paragraph><paragraph>[foo]</paragraph><paragraph>bar</paragraph>', '13']])
        );
      ...
      it('should move to next cell with an image', () => {
        setModelData(model, modelTable([['11[]', '<paragraph>foo</paragraph><image></image>']]));
        ...
        sinon.assert.calledOnce(domEvtDataStub.stopPropagation);
        expect(formatTable(getModelData(model))).to.equal(
          formattedModelTable([['11', '<paragraph>[foo</paragraph><image></image>]']])
        );
      });
      ...
      it('should move to next cell with an blockQuote', () => {
        ...
        setModelData(model, modelTable([['11[]', '<blockQuote><paragraph>foo</paragraph></blockQuote>']]));
        ...
        sinon.assert.calledOnce(domEvtDataStub.stopPropagation);
        expect(formatTable(getModelData(model))).to.equal(
          formattedModelTable([['11', '<blockQuote><paragraph>[foo]</paragraph></blockQuote>']])
        );
      });
    ...
    describe('on SHIFT+TAB', () => {
      ...
      it('should not move if caret is in first table cell', () => {
        setModelData(model, '<paragraph>foo</paragraph>' + modelTable([['[]11', '12']]));
      ...
      it('should move to previous cell with an image', () => {
        setModelData(model, modelTable([['<paragraph>foo</paragraph><image></image>', 'bar[]']]));
        ...
        sinon.assert.calledOnce(domEvtDataStub.stopPropagation);
        expect(formatTable(getModelData(model))).to.equal(
          formattedModelTable([['<paragraph>[foo</paragraph><image></image>]', 'bar']])
        );
      });
    });
  });
  ...
  describe('enter key', () => {
    ...
    it('should do nothing if table cell has already a block content', () => {
      setModelData(model, modelTable([['<paragraph>[]11</paragraph>']]));

      viewDocument.fire('enter', evtDataStub);

      sinon.assert.notCalled(editor.execute);
      expect(formatTable(getModelData(model))).to.equal(formattedModelTable([['<paragraph>[]11</paragraph>']]));
    });

    it('should do nothing if table cell with a block content is selected as a whole', () => {
      setModelData(model, modelTable([['<paragraph>[1</paragraph><paragraph>1]</paragraph>']]));
    ...
    it('should allow default behavior of Shift+Enter pressed', () => {
      ...
      sinon.assert.notCalled(editor.execute);
      expect(formatTable(getModelData(model))).to.equal(formattedModelTable([['[]11']]));
    });
  });
});
