...
describe('Table layout post-fixer', () => {
  ...
  describe('on insert table', () => {
    it('should add missing columns to tableRows that are shorter then the longest table row', () => {
      const parsed = parse(modelTable([['00'], ['10', '11', '12'], ['20', '21']]), model.schema);
      ...
      expect(formatTable(getModelData(model, { withoutSelection: true }))).to.equal(
        formattedModelTable([['00', '', ''], ['10', '11', '12'], ['20', '21', '']])
      );
    });

    it('should add missing columns to tableRows that are shorter then the longest table row (complex 1)', () => {
      const parsed = parse(
        modelTable([['00', { rowspan: 2, contents: '10' }], ['10', { colspan: 2, contents: '12' }], ['20', '21']]),
        model.schema
      );
    ...
    it('should fix the wrong rowspan attribute of a table cell inside the header', () => {
      ...
      expect(formatTable(getModelData(model, { withoutSelection: true }))).to.equal(
        formattedModelTable(
          [[{ rowspan: 2, contents: '00' }, { rowspan: 2, contents: '01' }, '02'], ['12'], ['20', '21', '22']],
          { headingRows: 2 }
        )
      );
    });
    ...
    it('should fix the wrong rowspan attribute of a table cell inside the body', () => {
      ...
      expect(formatTable(getModelData(model, { withoutSelection: true }))).to.equal(
        formattedModelTable(
          [['00', '01', '02'], [{ rowspan: 2, contents: '10' }, { rowspan: 2, contents: '11' }, '12'], ['22']],
          { headingRows: 1 }
        )
      );
    });
    ...
    it('should fix multiple tables', () => {
      const tableA = modelTable([['11'], ['21', '22']]);
      const tableB = modelTable([['aa', 'ab'], ['ba', 'bb']]);
      const tableC = modelTable([['xx'], ['yy', 'yy']]);
      ...
      const expectedTableA = formattedModelTable([['11', ''], ['21', '22']]);
      const expectedTableB = formattedModelTable([['aa', 'ab'], ['ba', 'bb']]);
      const expectedTableC = formattedModelTable([['xx', ''], ['yy', 'yy']]);
  ...
  describe('on collaboration', () => {
    ...
    it('should add empty cell to an added row (insert row vs insert column)', () => {
      _testExternal(
        ...
        formattedModelTable([['00', '', '01'], ['a', 'b', ''], ['10', '', '11']]),
        // Table will have empty column after undo.
        formattedModelTable([['00', '', '01'], ['10', '', '11']])
      );
    });
    ...
    it('should add empty cell when inserting column over a colspanned cell (insert column vs insert column)', () => {
      _testExternal(
        modelTable([[{ colspan: 3, contents: '00' }], ['10', '11', '12']]),
        writer => {
        ...
        // There should be empty cells added.
        formattedModelTable([[{ colspan: 4, contents: '00' }, ''], ['10', '', '11', '', '12']]),
        // Table will have empty column after undo.
        formattedModelTable([[{ colspan: 3, contents: '00' }, ''], ['10', '', '11', '12']])
      );
    });
    ...
    it('should insert table cell on undo (change table headers on row with rowspanned cell vs remove row)', () => {
      _testExternal(
        modelTable([['11', { rowspan: 2, contents: '12' }, '13'], ['21', '23'], ['31', '32', '33']]),
        writer => {
        ...
        formattedModelTable([['11', '12', '13'], ['31', '32', '33']], { headingRows: 1 }),
        formattedModelTable([['11', { rowspan: 2, contents: '12' }, '13', ''], ['31', '32', '33']])
      );
    });
    ...
    it('should insert empty table cell (remove row vs change table headers on row with rowspanned cell)', () => {
      _testExternal(
        ...
        formattedModelTable([['11', '12', '13', ''], ['31', '32', '33', '']], { headingRows: 1 }),
        formattedModelTable([['11', '12', '13', ''], ['21', '23', '', ''], ['31', '32', '33', '']], { headingRows: 1 })
      );
    });
    ...
    function _insertRow(writer, rowIndex, rowData) {
      ...
      const parsedTable = parse(modelTable([rowData]), model.schema);
