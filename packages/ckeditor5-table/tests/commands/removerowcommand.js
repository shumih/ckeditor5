  ...
  describe('execute()', () => {
    it('should remove a given row', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['00', '<paragraph>01[]</paragraph>'], ['20', '21']])
      );
    });
    ...
    it('should remove a given row from a table start', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['<paragraph>[]10</paragraph>', '11'], ['20', '21']])
      );
    });
    ...
    it('should change heading rows if removing a heading row', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['00', '<paragraph>01[]</paragraph>'], ['20', '21']], { headingRows: 1 })
      );
    });
    ...
    it("should move rowspaned cells to row below removing it's row", () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([[{ rowspan: 2, contents: '[]00' }, '01', '12'], ['22'], ['30', '31', '32']])
      );
    });
  });
});
