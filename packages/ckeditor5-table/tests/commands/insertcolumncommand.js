  ...
  describe('order=right', () => {
    ...
    describe('execute()', () => {
      ...
      it("should insert column in given table to the right of the selection's column (selection in block content)", () => {
        setData(model, modelTable([['11', '<paragraph>12[]</paragraph>', '13']]));

        command.execute();

        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['11', '<paragraph>12[]</paragraph>', '', '13']])
        );
      });
      ...
      it('should not update table heading columns attribute when inserting column after headings section', () => {
        setData(
          model,
          modelTable([['11', '12[]', '13'], ['21', '22', '23'], ['31', '32', '33']], { headingColumns: 2 })
        );
      ...
      it('should skip spanned columns', () => {
        setData(
          model,
          modelTable([['11[]', '12'], [{ colspan: 2, contents: '21' }], ['31', '32']], { headingColumns: 2 })
        );

        command.execute();

        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['11[]', '', '12'], [{ colspan: 3, contents: '21' }], ['31', '', '32']], {
            headingColumns: 3,
          })
        );
      });
