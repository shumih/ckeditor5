  ...
  describe('direction=right', () => {
    ...
    describe('isEnabled', () => {
      it('should be true if in cell that has sibling on the right', () => {
        setData(model, modelTable([['00[]', '01']]));
      ...
      it('should be true if in a cell that has sibling on the right with the same rowspan', () => {
        setData(model, modelTable([[{ rowspan: 2, contents: '00[]' }, { rowspan: 2, contents: '01' }]]));

        expect(command.isEnabled).to.be.true;
      });

      it('should be false if in a cell that has sibling but with different rowspan', () => {
        setData(model, modelTable([[{ rowspan: 2, contents: '00[]' }, { rowspan: 3, contents: '01' }]]));

        expect(command.isEnabled).to.be.false;
      });

      it('should be false when next cell is rowspanned', () => {
        setData(model, modelTable([['00', { rowspan: 3, contents: '01' }, '02'], ['10[]', '12'], ['20', '22']]));

        expect(command.isEnabled).to.be.false;
      });

      it('should be true when current cell is colspanned', () => {
        setData(model, modelTable([[{ colspan: 2, contents: '00[]' }, '02']]));
    ...
    describe('value', () => {
      ...
      it('should be set to mergeable sibling if in cell that has sibling on the right (selection in block content)', () => {
        setData(model, modelTable([['00', '<paragraph>[]01</paragraph>', '02']]));
    ...
    describe('execute()', () => {
      it('should merge table cells', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ colspan: 2, contents: '<paragraph>[00</paragraph><paragraph>01]</paragraph>' }]])
        );
      });
      ...
      it('should result in single empty paragraph if both cells are empty', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ colspan: 2, contents: '<paragraph>[]</paragraph>' }]])
        );
      });
      ...
      it('should result in single paragraph (other cell is empty)', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ colspan: 2, contents: '<paragraph>[foo]</paragraph>' }]])
        );
      });
      ...
      it('should not merge other empty blocks to single block', () => {
        ...
        setData(model, modelTable([['<block>[]</block>', '<block></block>']]));

        command.execute();

        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ colspan: 2, contents: '<block>[</block><block>]</block>' }]])
        );
      });
    });
  });
  ...
  describe('direction=down', () => {
    ...
    describe('isEnabled', () => {
      it('should be true if in cell that has mergeable cell in next row', () => {
        setData(model, modelTable([['00', '01[]'], ['10', '11']]));
      ...
      it('should be true if in a cell that has mergeable cell with the same colspan', () => {
        setData(model, modelTable([[{ colspan: 2, contents: '00[]' }, '02'], [{ colspan: 2, contents: '01' }, '12']]));

        expect(command.isEnabled).to.be.true;
      });

      it('should be false if in a cell that potential mergeable cell has different colspan', () => {
        setData(model, modelTable([[{ colspan: 2, contents: '00[]' }, '02'], [{ colspan: 3, contents: '01' }]]));
      ...
      it('should be false if mergeable cell is in other table section then current cell', () => {
        setData(model, modelTable([['00[]', '01'], ['10', '11']], { headingRows: 1 }));
    ...
    describe('value', () => {
      ...
      it('should be set to mergeable cell (selection in block content)', () => {
        setData(model, modelTable([['00'], ['<paragraph>10[]</paragraph>'], ['20']]));
      ...
      it('should be undefined if mergable cell is in other table section', () => {
        setData(
          model,
          modelTable([[{ rowspan: 2, contents: '00[]' }, '02'], ['12'], ['21', '22']], { headingRows: 2 })
        );
    ...
    describe('execute()', () => {
      ...
      it('should result in single empty paragraph if both cells are empty', () => {
        ...
        expect(formatTable(getData(model))).to.equal(formattedModelTable([[{ rowspan: 2, contents: '[]' }, ''], ['']]));
      });
      ...
      it('should result in single paragraph (other cell is empty)', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ rowspan: 2, contents: '[foo]' }, ''], ['']])
        );
      });
      ...
      it('should not merge other empty blocks to single block', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([[{ rowspan: 2, contents: '<block>[</block><block>]</block>' }, ''], ['']])
        );
      });
