  ...
  describe('order=below', () => {
    ...
    describe('execute()', () => {
      it('should insert row after current position', () => {
        ...
        expect(formatTable(getData(model))).to.equal(formattedModelTable([['00[]', '01'], ['', ''], ['10', '11']]));
      });
      ...
      it('should insert row after current position (selection in block content)', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['00'], ['<paragraph>[]10</paragraph>'], [''], ['20']])
        );
      });

      it('should update table heading rows attribute when inserting row in headings section', () => {
        setData(model, modelTable([['00[]', '01'], ['10', '11'], ['20', '21']], { headingRows: 2 }));

        command.execute();

        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['00[]', '01'], ['', ''], ['10', '11'], ['20', '21']], { headingRows: 3 })
        );
      });
      ...
      it('should not update table heading rows attribute when inserting row after headings section', () => {
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['00', '01'], ['10[]', '11'], ['', ''], ['20', '21']], { headingRows: 2 })
        );
      });
      ...
      it('should not expand rowspan of a cell that does not overlaps inserted rows', () => {
        setData(
          model,
          modelTable([[{ rowspan: 2, contents: '00' }, '01', '02'], ['11[]', '12'], ['20', '21', '22']], {
            headingColumns: 3,
            headingRows: 1,
          })
        );
        ...
        expect(formatTable(getData(model))).to.equal(
          formattedModelTable(
            [[{ rowspan: 2, contents: '00' }, '01', '02'], ['11[]', '12'], ['', '', ''], ['20', '21', '22']],
            { headingColumns: 3, headingRows: 1 }
          )
        );
      });
      ...
      it('should properly calculate columns if next row has colspans', () => {
        setData(
          model,
          modelTable([[{ rowspan: 2, contents: '00' }, '01', '02'], ['11[]', '12'], [{ colspan: 3, contents: '20' }]], {
            headingColumns: 3,
            headingRows: 1,
          })
        );
  ...
  describe('order=above', () => {
    ...
    describe('execute()', () => {
      ...
      it('should insert row at the end of a table', () => {
        setData(model, modelTable([['00', '01'], ['10', '11'], ['20[]', '21']]));

        command.execute();

        expect(formatTable(getData(model))).to.equal(
          formattedModelTable([['00', '01'], ['10', '11'], ['', ''], ['20[]', '21']])
        );
      });
