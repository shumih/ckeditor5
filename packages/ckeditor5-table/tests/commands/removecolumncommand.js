...
describe('RemoveColumnCommand', () => {
  ...
  describe('isEnabled', () => {
    ...
    it('should be false if selection is inside table with one column only', () => {
      setData(model, modelTable([['00'], ['10[]'], ['20[]']]));
  ...
  describe('execute()', () => {
    it('should remove a given column', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['00', '02'], ['<paragraph>10[]</paragraph>', '12'], ['20', '22']])
      );
    });
    ...
    it('should remove a given column from a table start', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['<paragraph>[]01</paragraph>'], ['11'], ['21']])
      );
    });

    it('should change heading columns if removing a heading column', () => {
      setData(model, modelTable([['00', '01'], ['[]10', '11'], ['20', '21']], { headingColumns: 2 }));

      command.execute();

      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([['01'], ['<paragraph>[]11</paragraph>'], ['21']], { headingColumns: 1 })
      );
    });
    ...
    it('should decrease colspan of cells that are on removed column', () => {
      ...
      expect(formatTable(getData(model))).to.equal(
        formattedModelTable([[{ colspan: 2, contents: '[]00' }, '03'], ['10', '13'], ['21', '22', '23']])
      );
    });
  });
});
