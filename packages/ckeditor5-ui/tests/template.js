  ...
  describe('render()', () => {
        ...
        describe('object', () => {
          it('renders with static and bound attributes', () => {
            ...
            expect(normalizeHtml(el.outerHTML)).to.equal(
              '<p style="background-color:yellow;height:10px;width:10px"></p>'
            );
            ...
            expect(normalizeHtml(el.outerHTML)).to.equal(
              '<p style="background-color:green;height:10px;width:20px"></p>'
            );
          });
    ...
    describe('children', () => {
      ...
      it('creates multiple child Text Nodes', () => {
        const el = new Template({
          tag: 'p',
          children: ['a', 'b', { text: 'c' }, 'd', { text: ['e', 'f'] }],
        }).render();
      ...
      it('renders view children', () => {
        ...
        expect(normalizeHtml(rendered.outerHTML)).to.equal(
          '<p><b class="v1"></b><b class="v2"></b><b class="v3"></b></p>'
        );
      ...
      it('renders template children', () => {
        ...
        const tpl = new Template({
          tag: 'p',
          children: [childTplA, childTplB, childTplC],
        });
        ...
        expect(normalizeHtml(tpl.render().outerHTML)).to.equal('<p><a></a><b></b><i>foo</i></p>');
      });
  ...
  describe('apply()', () => {
    ...
    describe('attributes', () => {
      ...
      it('manages existing attribute values ("class" vs. "non–class")', () => {
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal('<div class="a b default" x="bar"></div>');
      });
      ...
      describe('style', () => {
        ...
        it('applies when in an object syntax', () => {
          ...
          expect(normalizeHtml(el.outerHTML)).to.equal(
            '<p style="background-color:green;float:left;height:10px;width:20px"></p>'
          );
        });
    ...
    describe('children', () => {
      ...
      it('should work for deep DOM structure with bindings and event listeners', () => {
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal('<div><a class="a1 a2">a</a><b class="b1 b2">b</b></div>');
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal(
          '<div class="applied-parent-qux" id="BAR">' +
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal(
          '<div class="applied-parent-qux" id="UPDATED">' +
  ...
  describe('revert()', () => {
    ...
    describe('text', () => {
      it('should revert textContent to the initial value', () => {
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal('<a>bar<b>qux</b></a>');

        tpl.revert(el);

        expect(normalizeHtml(el.outerHTML)).to.equal('<a>a<b>b</b></a>');
      });
      ...
      it('should remove bindings', () => {
        ...
        tpl.apply(el);
        expect(normalizeHtml(el.outerHTML)).to.equal('<a>foo<b>bar</b></a>');

        observable.foo = 'abc';
        expect(normalizeHtml(el.outerHTML)).to.equal('<a>foo<b>abc</b></a>');
    ...
    describe('attributes', () => {
      it('should revert attributes to the initial values', () => {
        ...
        expect(normalizeHtml(el.outerHTML)).to.equal(
          '<a bar="ab1 ab2" baz="x" foo="af1">' + '<b bar="bb" foo="bf1"></b>' + '</a>'
        );

        tpl.revert(el);

        expect(normalizeHtml(el.outerHTML)).to.equal('<a bar="ab" foo="af">' + '<b bar="bb" foo="bf"></b>' + '</a>');
      });
      ...
      it('should remove bindings', () => {
        ...
        tpl.apply(el);
        expect(normalizeHtml(el.outerHTML)).to.equal(
          '<a bar="ab1 qux" foo="af1">' + '<b bar="bb" foo="bar"></b>' + '</a>'
        );

        observable.foo = 'x';
        observable.baz = 'y';
        expect(normalizeHtml(el.outerHTML)).to.equal('<a bar="ab1 y" foo="af1">' + '<b bar="bb" foo="x"></b>' + '</a>');
      ...
      describe('style', () => {
        ...
        it('should remove bindings', () => {
          ...
          tpl.revert(el);
          expect(normalizeHtml(el.outerHTML)).to.equal(
            '<a style="font-weight:bold">' + '<b style="color:red"></b>' + '</a>'
          );
  ...
  describe('bind()', () => {
    ...
    describe('event', () => {
      ...
      it('accepts plain binding', () => {
        ...
        sinon.assert.calledWithExactly(spy, sinon.match.has('name', 'a'), sinon.match.has('target', el));
      });
      ...
      it('accepts an array of event bindings', () => {
        ...
        sinon.assert.calledWithExactly(spy1, sinon.match.has('name', 'a'), sinon.match.has('target', el));
        sinon.assert.calledWithExactly(spy2, sinon.match.has('name', 'b'), sinon.match.has('target', el));
      });
      ...
      it('accepts DOM selectors', () => {
        ...
        expect(spy1.firstCall.calledWithExactly(sinon.match.has('name', 'a'), sinon.match.has('target', el.firstChild)))
          .to.be.true;
        ...
        expect(spy2.firstCall.calledWithExactly(sinon.match.has('name', 'b'), sinon.match.has('target', el.lastChild)))
          .to.be.true;
        ...
        expect(
          spy1.secondCall.calledWithExactly(
      ...
      it('accepts function callbacks', () => {
        ...
        sinon.assert.calledWithExactly(spy1, sinon.match.has('target', el));

        sinon.assert.calledWithExactly(spy2, sinon.match.has('target', el.firstChild));
      });
      ...
      it('supports event delegation', () => {
        ...
        sinon.assert.calledWithExactly(spy, sinon.match.has('name', 'a'), sinon.match.has('target', el.firstChild));
      });
      ...
      describe('to', () => {
        ...
        it('allows binding attribute to the observable – array of bindings (HTMLElement attribute)', () => {
          setElement({
            tag: 'p',
            attributes: {
              class: ['ck-class', bind.to('foo'), bind.to('baz'), bind.to('foo', value => `foo-is-${value}`), 'ck-end'],
            },
            children: ['abc'],
          });
        ...
        it('allows binding attribute to the observable – falsy values', () => {
          ...
          observable.foo = 'bar';
          expect(normalizeHtml(el.outerHTML)).to.equal(
            '<p complex="bar" emptystring="bar" simple="bar" zero="0 bar">abc</p>'
          );
      ...
      describe('if', () => {
        ...
        it('allows binding attribute to the observable – array of bindings (HTMLElement attribute)', () => {
          setElement({
            tag: 'p',
            attributes: {
              class: ['ck-class', bind.if('foo', 'foo-set'), bind.if('bar', 'bar-not-set', value => !value), 'ck-end'],
            },
            children: ['abc'],
          });
    ...
    describe('text', () => {
      ...
      it('extends existing - bindings #3', () => {
        const el = extensionTest(
          {
            tag: 'p',
            children: [{ text: bind.to('foo') }, 'X'],
          },
          {
            children: [{ text: bind.to('baz') }, 'Y'],
          },
          '<p>bar quxXY</p>'
        );
        ...
        const el = extensionTest(
          {
            ...
            on: {
              click: bind.to('A'),
              'click@span': [bind.to('B'), bind.to(spy5)],
            },
          },
          {
            on: {
      ...
      it('creates new', () => {
        ...
        const el = extensionTest(
          ...
          },
          {
            on: {
              click: bind.to('A'),
              'click@span': [bind.to('B'), bind.to(spy3)],
            },
          },
          '<p><span></span></p>'
        );
