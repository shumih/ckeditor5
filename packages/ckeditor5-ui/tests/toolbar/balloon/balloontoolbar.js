...
describe('BalloonToolbar', () => {
  ...
  describe('show()', () => {
    ...
    beforeEach(() => {
      ...
      stubSelectionRects([backwardSelectionRect, forwardSelectionRect]);
    ...
    it('should attach the #_balloon to the last range in a case of multi-range forward selection', () => {
      ...
      expect(viewStringify(targetViewRange.root, targetViewRange, { ignoreRoot: true })).to.equal(
        '<p>bar</p><p>{bi}z</p>'
      );
    ...
    it('should ignore the zero-width orphan rect if there another one preceding it for the forward selection', () => {
      ...
      stubSelectionRects([forwardSelectionRect, { width: 0 }]);
