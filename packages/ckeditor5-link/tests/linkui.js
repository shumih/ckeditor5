...
describe('LinkUI', () => {
  ...
  describe('_showUI()', () => {
    ...
    it(
      'should add #actionsView to the balloon and attach the balloon to the link element when collapsed selection is inside ' +
        'that link',
      () => {
    ...
    it('should update balloon position when is switched in rotator to a visible panel', () => {
      ...
      const linkViewElement = editor.editing.view.document
        .getRoot()
        .getChild(0)
        .getChild(1);
      ...
      const newLinkViewElement = editor.editing.view.document
        .getRoot()
        .getChild(0)
        .getChild(0)
        .getChild(1);
    ...
    describe('response to ui#update', () => {
      ...
      it('hides the panel – editing a link, then the selection expands', () => {
        ...
        view.change(writer => {
          writer.setSelection(
            writer.createRange(writer.createPositionAt(text, 1), writer.createPositionAt(text, 2)),
            true
          );
        });
