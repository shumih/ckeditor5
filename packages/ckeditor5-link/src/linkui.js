...
import { isLinkElement, getExtension } from './utils';
...
export default class LinkUI extends Plugin {
  ...
  _createFormView() {
    const editor = this.editor;
    const linkCommand = editor.commands.get('link');
    const { uploadHandler, extensions } = editor.config.get('link');

    const formView = new LinkFormView(editor.locale, linkCommand.manualDecorators, extensions);

    formView.urlInputView.bind('value').to(linkCommand, 'value');

    // Form elements should be read-only when corresponding commands are disabled.
    formView.urlInputView.bind('isReadOnly').to(linkCommand, 'isEnabled', value => !value);
    formView.uploadStorageButtonView.bind('isEnabled').to(linkCommand, 'isEnabled', value => !value);
    ...
    formView.uploadStorageButtonView.on('done', async (_, files) => {
      ...
      if (url == null) {
        ...
        throw Error('CKeditor5: link.uploadHandler in configuration should return "string" or "null"');
      }

      editor.execute('link', url, getExtension(fileName), formView.getDecoratorSwitchesState());
      this._closeFormView();
    });
  ...
  _showUI(forceVisible = false) {
    ...
    if (!this._getSelectedLinkElement()) {
      ...
      if (forceVisible && typeof this._balloon.showStack === 'function') {
