...
describe('HeadingCommand', () => {
  ...
  describe('execute()', () => {
    ...
    it('should not rename blocks which cannot become headings (heading is not allowed in their parent)', () => {
      ...
      setData(
        model,
        '<paragraph>a[bc</paragraph>' + '<restricted><fooBlock></fooBlock></restricted>' + '<fooBlock>de]f</fooBlock>'
      );

      command.execute({ value: 'heading1' });

      expect(getData(model)).to.equal(
        '<heading1>a[bc</heading1>' + '<restricted><fooBlock></fooBlock></restricted>' + '<heading1>de]f</heading1>'
      );
    });
    ...
    it('should not rename blocks which cannot become headings (block is an object)', () => {
      ...
      setData(model, '<paragraph>a[bc</paragraph>' + '<image></image>' + '<paragraph>de]f</paragraph>');

      command.execute({ value: 'heading1' });

      expect(getData(model)).to.equal('<heading1>a[bc</heading1>' + '<image></image>' + '<heading1>de]f</heading1>');
    });
    ...
    describe('non-collapsed selection', () => {
      ...
      it('converts all elements where selection is applied', () => {
        ...
        expect(getData(model)).to.equal('<heading3>foo[</heading3><heading3>bar</heading3><heading3>baz]</heading3>');
      });
      ...
      it('does nothing to the elements with same option (#1)', () => {
        ...
        expect(getData(model)).to.equal('<heading1>[foo</heading1><heading1>bar]</heading1>');
      });
      ...
      it('does nothing to the elements with same option (#2)', () => {
        ...
        expect(getData(model)).to.equal('<heading1>[foo</heading1><heading1>bar</heading1><heading1>baz]</heading1>');
      });

      function test({ model: fromElement }, { model: toElement }) {
        it(`converts ${fromElement} to ${toElement} on non-collapsed selection`, () => {
          setData(model, `<${fromElement}>foo[bar</${fromElement}><${fromElement}>baz]qux</${fromElement}>`);

          command.execute({ value: toElement });

          expect(getData(model)).to.equal(`<${toElement}>foo[bar</${toElement}><${toElement}>baz]qux</${toElement}>`);
        });
      }
    });
  });
