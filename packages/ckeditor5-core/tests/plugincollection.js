...
describe('PluginCollection', () => {
  ...
  describe('load()', () => {
    ...
    it('should add collection items for loaded plugins', () => {
      ...
      return plugins.init([PluginA, PluginB]).then(() => {
    ...
    it('should load dependency plugins', () => {
      ...
      return plugins.init([PluginA, PluginC]).then(loadedPlugins => {
    ...
    it('should load dependency plugins defined by plugin names', () => {
      ...
      return plugins.init(['J']).then(loadedPlugins => {
        expect(getPlugins(plugins).length).to.equal(3);

        expect(getPluginNames(getPluginsFromSpy(spy))).to.deep.equal(['A', 'K', 'J'], 'order by plugins._add()');
        expect(getPluginNames(loadedPlugins)).to.deep.equal(['A', 'K', 'J'], 'order by returned value');
      });
    });
    ...
    it('should be ok when dependencies are loaded first', () => {
      ...
      return plugins.init([PluginA, PluginB, PluginC]).then(loadedPlugins => {
        expect(getPlugins(plugins).length).to.equal(3);
    ...
    it('should load deep dependency plugins', () => {
      ...
      return plugins.init([PluginD]).then(loadedPlugins => {
    ...
    it('should handle cross dependency plugins', () => {
      ...
      return plugins.init([PluginA, PluginE]).then(loadedPlugins => {
        expect(getPlugins(plugins).length).to.equal(3);
    ...
    it('should load grand child classes', () => {
      ...
      return plugins.init([PluginG]).then(() => {
    ...
    it('should load plugin which is a simple function', () => {
      ...
      return plugins.init([pluginAsFunction]).then(() => {
        expect(getPlugins(plugins).length).to.equal(1);
      });
    });
    ...
    it('should set the `editor` property on loaded plugins', () => {
      ...
      return plugins.init([PluginA, PluginB, pluginAsFunction, Y]).then(() => {
    ...
    it('should load chosen plugins (plugins and removePlugins are constructors)', () => {
      ...
      return plugins.init([PluginA, PluginB, PluginC], [PluginA]).then(() => {
        expect(getPlugins(plugins).length).to.equal(2);
    ...
    it('should load chosen plugins (plugins are constructors, removePlugins are names)', () => {
      ...
      return plugins.init([PluginA, PluginB, PluginC], ['A']).then(() => {
        expect(getPlugins(plugins).length).to.equal(2);
    ...
    it('should load chosen plugins (plugins and removePlugins are names)', () => {
      ...
      return plugins.init(['A', 'B', 'C'], ['A']).then(() => {
        expect(getPlugins(plugins).length).to.equal(2);
    ...
    it('should load chosen plugins (plugins are names, removePlugins are constructors)', () => {
      ...
      return plugins.init(['A', 'B', 'C'], [PluginA]).then(() => {
        expect(getPlugins(plugins).length).to.equal(2);
    ...
    it('should load chosen plugins (plugins are names, removePlugins contains an anonymous plugin)', () => {
      ...
      return plugins.init([AnonymousPlugin, 'A', 'B'], [AnonymousPlugin]).then(() => {
        expect(getPlugins(plugins).length).to.equal(2);
    ...
    it('should reject when loaded plugin requires not allowed plugins', () => {
      ...
      return (
        plugins
          .init([PluginA, PluginB, PluginC, PluginD], [PluginA, PluginB])
    ...
    it('should reject when loading more than one plugin with the same name', () => {
      ...
      return plugins
        .init([PluginFoo, AnotherPluginFoo])
        .then(() => {
    ...
    it(
      'should reject when loading more than one plugin with the same name' +
    ...
    it('should get plugin from external plugins instead of creating new instance', async () => {
      ...
      expect(plugins.get(PluginA))
        .to.equal(externalPlugins.get(PluginA))
        .to.instanceof(PluginA);
    });
    ...
    it('should get dependency of plugin from external plugins instead of creating new instance', async () => {
      ...
      expect(getPlugins(plugins)).to.length(2);
      expect(plugins.get(PluginB))
        .to.equal(externalPlugins.get(PluginB))
        .to.instanceof(PluginB);
  ...
  describe('get()', () => {
    it('retrieves plugin by its constructor', () => {
      ...
      return plugins.init([SomePlugin]).then(() => {
    ...
    it('throws if plugin cannot be retrieved by name', () => {
      ...
      return plugins.init([]).then(() => {
        expectToThrowCKEditorError(() => plugins.get('foo'), /^plugincollection-plugin-not-loaded:/, editor, {
          plugin: 'foo',
        });
      });
    });
    ...
    it('throws if plugin cannot be retrieved by class', () => {
      ...
      return plugins.init([]).then(() => {
        expectToThrowCKEditorError(() => plugins.get(SomePlugin), /^plugincollection-plugin-not-loaded:/, editor, {
          plugin: 'foo',
        });
      });
    });
  ...
  describe('destroy()', () => {
    ...
    it('waits until all plugins are destroyed', () => {
      ...
      return plugins
        .init([AsynchronousPluginA, AsynchronousPluginB])
  ...
  describe('iterator', () => {
    ...
    it('returns only plugins by constructors', () => {
      ...
      return plugins.init([SomePlugin1, SomePlugin2]).then(() => {
        const pluginConstructors = Array.from(plugins).map(entry => entry[0]);
...
function getPlugins(pluginCollection) {
  return Array.from(pluginCollection).map(entry => entry[1]); // Get instances.
}
