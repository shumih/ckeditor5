...
describe('Editor', () => {
  ...
  describe('locale', () => {
    it('should use Context#locale and Context#t', () => {
      ...
      expect(editor.locale)
        .to.equal(editor._context.locale)
        .to.instanceof(Locale);
  ...
  describe('create()', () => {
    ...
    it('loads plugins', () => {
      return TestEditor.create({ plugins: [PluginA] }).then(editor => {
    ...
    it('fires ready event', () => {
      ...
      return TestEditor.create({ plugins: [EventWatcher] }).then(() => {
...
function getPlugins(editor) {
  return Array.from(editor.plugins).map(entry => entry[1]); // Get instances.
}
