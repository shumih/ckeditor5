...
describe('ClassicTestEditor', () => {
  ...
  describe('create()', () => {
    it('creates an instance of editor', () => {
      return ClassicTestEditor.create(editorElement, { foo: 1 }).then(editor => {
    ...
    it('loads data from the editor element', () => {
      ...
      return ClassicTestEditor.create(editorElement, { plugins: [PluginTextInRoot] }).then(editor => {
    ...
    it('fires all events in the right order', () => {
      ...
      return ClassicTestEditor.create(editorElement, {
        ...
        expect(fired).to.deep.equal(['ready-classictesteditorui', 'ready-datacontroller', 'ready-classictesteditor']);
    ...
    it('initializes the data controller with `config.initialData` if this option is provided', () => {
      return ClassicTestEditor.create(editorElement, { initialData: '<p>foo</p>', plugins: [Paragraph] }).then(
        editor => {
    ...
    it('initializes the data controller with the data from the source element', () => {
      ...
      return ClassicTestEditor.create(editorElement, { plugins: [Paragraph] }).then(editor => {
        expect(editor.getData()).to.equal('<p>foo</p>');
    ...
    it('initializes the data controller with the data from the first argument if it is a string', () => {
      return ClassicTestEditor.create('<p>foo</p>', { plugins: [Paragraph] }).then(editor => {
        expect(editor.getData()).to.equal('<p>foo</p>');
    ...
    it('throws when the data is passed from as the first argument and as a `config.initialData` at the same time', () => {
      return ClassicTestEditor.create('<p>foo</p>', { initialData: '<p>bar</p>' }).then(
        () => {
