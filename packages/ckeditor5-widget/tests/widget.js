...
describe('Widget', () => {
  ...
  beforeEach(() => {
    ...
    return VirtualTestEditor.create({ plugins: [Widget, Typing] }).then(newEditor => {
      editor = newEditor;
      model = editor.model;
  ...
  it('should toggle selected class', () => {
    ...
    expect(getViewData(view)).to.equal('<p>{}foo</p><div class="ck-widget" contenteditable="false">foo<b></b></div>');
  });
  ...
  describe('delete integration', () => {
    ...
    it('should prevent default behaviour and stop event propagation', () => {
      ...
      viewDocument.fire(
        'delete',
        new DomEventData(viewDocument, domEventDataMock, { direction: 'forward', unit: 'character', sequence: 0 })
      );
    ...
    it('does nothing when editor when read only mode is enabled (delete)', () => {
      ...
      viewDocument.fire(
        'delete',
        new DomEventData(viewDocument, domEventDataMock, { direction: 'backward', unit: 'character', sequence: 0 })
      );
  ...
  describe('selection handler', () => {
    ...
    it('should select a widget on mouse click', () => {
      ...
      const viewWidgetSelectionHandler = viewDocument
        .getRoot()
        .getChild(1)
        .getChild(0);
    ...
    it('works fine with a widget that contains more children', () => {
      setModelData(model, '<widget>' + '<nested>foo bar</nested>' + '<widget></widget>' + '</widget>');
    ...
    it('should select widget in editable', () => {
      ...
      const widgetInEditable = viewDocument
        .getRoot()
        .getChild(0)
        .getChild(0)
        .getChild(0);
