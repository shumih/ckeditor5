...
export default class ResizeState {
  ...
  begin(domResizeHandle, domHandleHost, domResizeHost) {
    ...
    this._referenceCoordinates = getAbsoluteBoundaryPoint(
      domHandleHost,
      getOppositePosition(this.activeHandlePosition)
    );
...
function calculateHostPercentageWidth(domResizeHost, resizeHostRect) {
  ...
  const parentWidth = parseFloat(
    domResizeHostParent.ownerDocument.defaultView.getComputedStyle(domResizeHostParent).width
  );
