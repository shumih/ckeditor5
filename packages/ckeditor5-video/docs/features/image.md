...
* {@link module:image/image~Video} implements basic support for images.
* {@link module:image/imagetoolbar~Videotoolbar} adds the image feature's contextual toolbar.
* {@link module:image/videocaption~Videocaption} adds support for captions.
* {@link module:image/imagestyle~Videostyle} adds support for image styles.
* {@link module:image/imagetextalternative~Videotextalternative} adds support for adding text alternative.
* {@link module:image/imageupload~Videoupload} adds support for uploading dropped or pasted images (see: {@link features/image-upload Video upload}).
* {@link module:image/imageresize~Videoresize} adds support for resizing images.
...
The {@link module:image/image~Video} feature adds support for plain images with just the `alt` attribute set. This translates to the following HTML:
...
## Video contextual toolbar

The {@link module:image/imagetoolbar~Videotoolbar} plugin introduces a contextual toolbar for images. The toolbar appears when an image is selected and can be configured to contain any buttons you want. Usually, these will be image-related options such as the text alternative (which is introduced by the base image plugin) button and [image styles buttons](#image-styles).
...
The {@link module:image/videocaption~Videocaption} plugin adds support for image captions:
...
This is what the {@link module:image/imagestyle~Videostyle} feature is designed for.
...
```html
<figure class="image"><video src="..." alt="..."></figure>
```

A side image:

```html
<figure class="image image-style-side"><video src="..." alt="..."></figure>
```
...
The available image styles can be configured using the {@link module:image/image~VideoConfig#styles `image.styles`} option.
...
ClassicEditor
	.create( document.querySelector( '#editor' ), {
		image: {
			...
			toolbar: [ 'videoTextAlternative', '|', 'imageStyle:alignLeft', 'imageStyle:full', 'imageStyle:alignRight' ],
...
The code sample above uses predefined image styles: `'full'`, `'alignLeft'` and `'alignRight'`. The latter two apply, respectively, the `.video-style-align-left` and  `.video-style-align-right` classes to the `<figure>` element.
...
You can find advanced examples in the {@link module:image/image~VideoConfig#styles `image.styles`} configuration option documentation.

<!-- TODO (live example)... -->

## Video upload

See the {@link features/image-upload Video upload} guide.

## Responsive images

Support for responsive images in CKEditor 5 is brought by the {@link features/easy-image Easy Video} feature without any additional configuration. Learn more how to use the feature in your project in the {@link features/easy-image#responsive-images Easy Video integration} guide.
...
It is implemented by the {@link module:image/imageresize~Videoresize} plugin and enables four "resize handles" displayed over the selected image. The user can freely resize the image by dragging them. The feature can be configured to use either percentage (default) or pixel values.

{@snippet features/image-resize}

### Enabling image resizing

The image resize feature is not enabled by default in any of the editor builds. In order to enable it, you need to load the {@link module:image/imageresize~Videoresize} plugin. Read more in the [Installation](#installation) section.
...
.ck-content .video.video_resized > figcaption {
	display: block;
}
```
...
However, there are cases where pixel values may be preferred. You can thus configure the editor to use them by setting the {@link module:image/image~VideoConfig#resizeUnit `config.video.resizeUnit`} option:
...
```js
import Video from '@ckeditor/ckeditor5-image/src/image';
import Videotoolbar from '@ckeditor/ckeditor5-image/src/imagetoolbar';
import Videocaption from '@ckeditor/ckeditor5-image/src/videocaption';
import Videostyle from '@ckeditor/ckeditor5-image/src/imagestyle';
import Videoresize from '@ckeditor/ckeditor5-image/src/imageresize';

ClassicEditor
	.create( document.querySelector( '#editor' ), {
		plugins: [ Video, Videotoolbar, Videocaption, Videostyle, Videoresize ],
		image: {
			toolbar: [ 'videoTextAlternative', '|', 'imageStyle:full', 'imageStyle:side' ]
		}
	} )
	.then( ... )
	.catch( ... );
...
The {@link module:image/image~Video} plugin registers:

* The `'videoTextAlternative'` button.
* The {@link module:image/imagetextalternative/imagetextalternativecommand~VideoTextAlternativeCommand `'videoTextAlternative'` command}
* The {@link module:image/image/videoinsertcommand~Videoinsertcommand `'imageInsert'` command} which accepts a source (e.g. an URL) of an image to insert.

The {@link module:image/imagestyle~Videostyle} plugin registers:

* A button for each defined style &mdash; e.g. `'imageStyle:full'` and `'imageStyle:side'`.
* The {@link module:image/imagestyle/imagestylecommand~VideoStyleCommand `'imageStyle'` command} which accepts a value based on the {@link module:image/image~VideoConfig#styles `image.styles`} configuration option (e.g. `'full'` and `'side'`):

	```js
	editor.execute( 'imageStyle', { value: 'side' } );
	```

The {@link module:image/imageupload~Videoupload} plugin registers:

* The `'videoUpload'` button which opens the native file browser to let you upload a file directly from your disk.
* The {@link module:image/imageupload/imageuploadcommand~VideoUploadCommand `'videoUpload'` command} which accepts the file to upload.

The {@link module:image/imageresize~Videoresize} plugin registers:

* The {@link module:image/imageresize/imageresizecommand~VideoResizeCommand `'imageResize'` command} which accepts the target width.
