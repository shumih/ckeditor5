...
ClassicEditor.create(document.querySelector('#editor'), {
  ...
  autosave.adapter = {
    save() {
      ...
      return wait(1000).then(() => console.log(`${getTime()} Saved content: ${data}`));
    },
  };

  autosave.listenTo(autosave, 'change:state', (evt, propName, newValue, oldValue) =>
    console.log(`${getTime()} Changed state: ${oldValue} -> ${newValue}`)
  );
});
