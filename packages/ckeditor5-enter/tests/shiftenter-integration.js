...
describe('ShiftEnter integration', () => {
  ...
  beforeEach(() => {
    ...
    return ClassicEditor.create(div, { plugins: [Paragraph, ShiftEnter] }).then(newEditor => {
      editor = newEditor;
  ...
  it('loads correct data', () => {
    expect(getModelData(model, options)).to.equal(
      '<paragraph>First line.<softBreak></softBreak>Second line.</paragraph>'
    );
