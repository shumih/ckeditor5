  ...
  parse(node, order) {
    ...
    if (order.length) {
      if (order.length != ranges.length) {
        throw new Error(
          `Parse error - there are ${ranges.length} ranges found, but ranges order array contains ${
            order.length
          } elements.`
        );
      }
    ...
    if (node.is('text')) {
      ...
      for (const item of brackets) {
        ...
        if (text) {
          ...
          if (
            (!this.sameSelectionCharacters && item.bracket == TEXT_RANGE_START_TOKEN) ||
...
function _convertElement(viewDocument, viewElement) {
  ...
  const newElement = ElementConstructor
    ? new ElementConstructor(viewDocument, info.name)
    : new ViewElement(viewDocument, info.name);
