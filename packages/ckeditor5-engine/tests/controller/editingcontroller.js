...
describe('EditingController', () => {
  describe('constructor()', () => {
    ...
    it('should create controller with properties', () => {
      expect(editing)
        ...
        .that.equals(model);
      expect(editing)
        .to.have.property('view')
        .that.is.instanceof(View);
      expect(editing)
        .to.have.property('mapper')
        .that.is.instanceof(Mapper);
      expect(editing)
        .to.have.property('downcastDispatcher')
        .that.is.instanceof(DowncastDispatcher);
    ...
    it('should bind view roots to model roots', () => {
      ...
      expect(editing.view.document.getRoot().name)
        .to.equal(modelRoot.name)
        .to.equal('$root');
    });
  });
  ...
  describe('conversion', () => {
    ...
    beforeEach(() => {
      ...
      const modelData = new ModelDocumentFragment(
        parse(
    ...
    it('should convert delete', () => {
      model.change(writer => {
        writer.remove(ModelRange._createFromPositionAndShift(model.document.selection.getFirstPosition(), 1));
      ...
      listener.listenTo(editing.view.document, 'selectionChange', () => {
        setTimeout(() => {
          expect(getModelData(model)).to.equal(
            '<paragraph>foo</paragraph>' + '<paragraph></paragraph>' + '<paragraph>b[a]r</paragraph>'
          );
    ...
    it('should convert adding marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p>f<span>oo</span></p><p></p><p><span>ba</span>r</p>'
      );
    });
    ...
    it('should convert removing marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal('<p>foo</p><p></p><p>bar</p>');
    });
    ...
    it('should convert changing marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p><span>fo</span>o</p><p></p><p>bar</p>'
      );
    });
    ...
    it('should convert insertion into marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p>f<span>oo</span></p><p><span>xyz</span></p><p><span>ba</span>r</p>'
      );
    });
    ...
    it('should convert move to marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p>f<span>oor</span></p><p></p><p><span>ba</span></p>'
      );
    });
    ...
    it('should convert move from marker', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p>f</p><p></p><p><span>ba</span>roo</p>'
      );
    });
    ...
    it('should convert the whole marker move', () => {
      ...
      expect(getViewData(editing.view, { withoutSelection: true })).to.equal(
        '<p></p><p>f<span>oo</span></p><p>bar</p>'
      );
    });
