...
describe('DocumentSelection', () => {
  ...
  describe('default range', () => {
    ...
    it('should skip element when you can not put selection', () => {
      ...
      doc = model.document;
      root = doc.createRoot();
      root._insertChild(0, [new Element('img'), new Element('p', [], new Text('foobar'))]);
  ...
  describe('markers', () => {
    ...
    it('should add markers to the collection when selection is inside the marker range', () => {
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [2, 2]), writer.createPositionFromPath(root, [2, 4]))
        );
    ...
    it('should update markers after selection change', () => {
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [2, 1]), writer.createPositionFromPath(root, [2, 2]))
        );
      ...
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [2, 4]), writer.createPositionFromPath(root, [2, 5]))
        );
      });
    ...
    it('should not add marker when collapsed selection is on the marker left bound', () => {
      model.change(writer => {
        ...
        writer.addMarker('marker', {
          range: writer.createRange(writer.createPositionFromPath(root, [2, 2])),
          usingOperation: false,
        });
      });
    ...
    it('should not add marker when collapsed selection is on the marker right bound', () => {
      model.change(writer => {
        writer.setSelection(writer.createRange(writer.createPositionFromPath(root, [2, 4])));
    ...
    it('should add marker when non-collapsed selection is inside a marker and touches the left bound', () => {
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [2, 1]), writer.createPositionFromPath(root, [2, 3]))
        );
    ...
    it('should add marker when non-collapsed selection is inside a marker and touches the right bound', () => {
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [2, 2]), writer.createPositionFromPath(root, [2, 5]))
        );
    ...
    it('should add marker of selected widget', () => {
      ...
      model.change(writer => {
        writer.setSelection(
          writer.createRange(writer.createPositionFromPath(root, [0]), writer.createPositionFromPath(root, [1]))
        );

        writer.addMarker('marker', {
          range: writer.createRange(writer.createPositionFromPath(root, [0]), writer.createPositionFromPath(root, [1])),
          usingOperation: false,
        });
      });
  ...
  describe('_setTo()', () => {
    ...
    it('should refresh attributes – integration test for #630', () => {
      ...
      expect(getData(model)).to.equal('f<$text italic="true">o</$text><$text bold="true">ob[]a</$text>r');
    });
  });
  ...
  describe('attributes', () => {
    ...
    beforeEach(() => {
      ...
      root._appendChild([new Element('p', [], new Text('foobar')), new Element('p', [], [])]);
    ...
    describe('are updated on a direct range change', () => {
      beforeEach(() => {
        root._insertChild(0, [
          ...
          new Text('b', { b: true }),
          new Text('c', { c: true }),
          new Element('p', [], [new Text('d', { d: true })]),
          new Element('p', { p: true }),
          new Text('e', { e: true }),
        ]);
      });
  ...
  describe('refreshing selection data', () => {
    it('should be up to date before post fixers', () => {
      ...
      model.change(writer => {
        ...
        writer.addMarker('marker', {
          range: writer.createRange(writer.createPositionFromPath(p, [1]), writer.createPositionFromPath(p, [5])),
          usingOperation: false,
        });
  ...
  describe('after applying an operation should get updated and fire events', () => {
    ...
    describe('InsertOperation', () => {
      it('before selection', () => {
        ...
        model.applyOperation(new InsertOperation(new Position(root, [0, 1]), 'xyz', doc.version));
      ...
      it('inside selection', () => {
        ...
        model.applyOperation(new InsertOperation(new Position(root, [1, 0]), 'xyz', doc.version));
    ...
    describe('MoveOperation', () => {
      it('move range from before a selection', () => {
        ...
        model.applyOperation(new MoveOperation(new Position(root, [0, 0]), 2, new Position(root, [2]), doc.version));
      ...
      it('moved into before a selection', () => {
        ...
        model.applyOperation(new MoveOperation(new Position(root, [2]), 2, new Position(root, [0, 0]), doc.version));
      ...
      it('move range from inside of selection', () => {
        ...
        model.applyOperation(new MoveOperation(new Position(root, [1, 0]), 2, new Position(root, [2]), doc.version));
      ...
      it('moved range intersects with selection', () => {
        ...
        model.applyOperation(new MoveOperation(new Position(root, [1, 3]), 2, new Position(root, [4]), doc.version));
    ...
    describe('MoveOperation to graveyard', () => {
      it('fix selection range if it ends up in graveyard - collapsed selection', () => {
        ...
        model.applyOperation(
          new MoveOperation(new Position(root, [1, 2]), 2, new Position(doc.graveyard, [0]), doc.version)
        );
      ...
      it('fix selection range if it ends up in graveyard - parent of non-collapsed selection is moved', () => {
        ...
        model.applyOperation(
          new MoveOperation(new Position(root, [1]), 2, new Position(doc.graveyard, [0]), doc.version)
        );
      ...
      it('fix selection range if it ends up in graveyard - whole content removed', () => {
        model.applyOperation(
          new MoveOperation(new Position(root, [0]), 3, new Position(doc.graveyard, [0]), doc.version)
        );

        expect(selection.getFirstPosition().path).to.deep.equal([0]);

        model.applyOperation(new InsertOperation(new Position(root, [0]), new Element('p'), doc.version));
      ...
      it('handles multi-range selection in a text node by merging it into one range (resulting in collapsed ranges)', () => {
        ...
        model.applyOperation(
          new MoveOperation(new Position(root, [1, 1]), 4, new Position(doc.graveyard, [0]), doc.version)
        );
      ...
      it('handles multi-range selection on object nodes by merging it into one range (resulting in non-collapsed ranges)', () => {
        ...
        model.applyOperation(
          new MoveOperation(new Position(root, [0, 0]), 2, new Position(doc.graveyard, [0]), doc.version)
        );
    ...
    it('`DocumentSelection#change:range` event should be fire once even if selection contains multi-ranges', () => {
      ...
      selection._setTo([Range._createIn(root.getNodeByPath([0])), Range._createIn(root.getNodeByPath([1]))]);

      spyRange = sinon.spy();
      selection.on('change:range', spyRange);

      model.applyOperation(new InsertOperation(new Position(root, [0]), 'xyz #1', doc.version));
