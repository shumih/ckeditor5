...
describe('Selection', () => {
    ...
    it('should throw an error if added ranges intersects', () => {
      expectToThrowCKEditorError(
        () => {
          ...
          new Selection([liveRange, new Range(new Position(root, [0, 4]), new Position(root, [1, 2]))]);
        },
  ...
  describe('setTo()', () => {
    ...
    it('should throw an error if added ranges intersects', () => {
      expectToThrowCKEditorError(
        () => {
          selection.setTo([liveRange, new Range(new Position(root, [0, 4]), new Position(root, [1, 2]))]);
        },
        /model-selection-range-intersects/,
        model
      );
  ...
  describe('setFocus()', () => {
    ...
    it('modifies only the last range', () => {
      ...
      selection.setTo([new Range(startPos1, endPos1), new Range(startPos2, endPos2)]);
  ...
  describe('getSelectedBlocks()', () => {
    ...
    it('gets deeper into the tree', () => {
      ...
      expect(stringifyBlocks(doc.selection.getSelectedBlocks())).to.deep.equal(['p#a', 'p#b', 'p#c', 'p#d']);
    });
    ...
    it('gets deeper into the tree (start deeper)', () => {
      ...
      expect(stringifyBlocks(doc.selection.getSelectedBlocks())).to.deep.equal(['p#c', 'p#d']);
    });
  ...
  describe('attributes interface', () => {
    ...
    beforeEach(() => {
      root._insertChild(0, [new Element('p', [], new Text('foobar')), new Element('p', [], [])]);
