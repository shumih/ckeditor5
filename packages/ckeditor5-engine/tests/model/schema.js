...
describe('Schema', () => {
  ...
  beforeEach(() => {
    ...
    root1 = new Element('$root', null, [
      ...
      new Element('blockQuote', null, [new Element('paragraph', null, 'foo')]),
    ]);
  ...
  describe('checkMerge()', () => {
    ...
    it('returns false if a block cannot be merged with other block (disallowed element is the first child)', () => {
      const paragraph = new Element('paragraph', null, [new Text('xyz')]);
    ...
    it('returns false if a block cannot be merged with other block (disallowed element is not the first child)', () => {
      const paragraph = new Element('paragraph', null, [new Text('foo')]);
      const blockQuote = new Element('blockQuote', null, [new Text('bar', { bold: true }), new Text('xyz'), paragraph]);
      const listItem = new Element('listItem');
    ...
    it('returns true if a block can be merged with other block', () => {
      const listItem = new Element('listItem');
      const listItemToMerge = new Element('listItem', null, [new Text('xyz')]);
    ...
    it('return true if two elements between the position can be merged', () => {
      const listItem = new Element('listItem', null, [new Text('foo')]);
      const listItemToMerge = new Element('listItem', null, [new Text('bar')]);

      // eslint-disable-next-line no-new
      new Element('$root', null, [listItem, listItemToMerge]);
    ...
    it('throws an error if there is no element before the position', () => {
      ...
      // eslint-disable-next-line no-new
      new Element('$root', null, [listItem]);
    ...
    it('throws an error if the node before the position is not the element', () => {
      ...
      // eslint-disable-next-line no-new
      new Element('$root', null, [new Text('bar'), listItem]);
    ...
    it('throws an error if the node after the position is not the element', () => {
      ...
      // eslint-disable-next-line no-new
      new Element('$root', null, [listItem, new Text('bar')]);
  ...
  describe('getValidRanges()', () => {
    ...
    it('should not break a range if children are not allowed to have the attribute', () => {
      ...
      testValidRangesForAttribute('[<p>foo</p><p>bar</p>]', 'foo', '[<p>foo</p><p>bar</p>]');
    });

    it('should search deeply', () => {
      schema.extend('$text', { allowAttributes: 'bold', allowIn: 'img' });

      testValidRangesForAttribute('[<p>foo<img>xxx</img>bar</p>]', 'bold', '<p>[foo]<img>[xxx]</img>[bar]</p>');
    });
    ...
    it('should correctly handle a range which ends in a disallowed position', () => {
      ...
      testValidRangesForAttribute('[<p>foo<img>xx]x</img>bar</p>', 'bold', '<p>[foo]<img>xxx</img>bar</p>');
    });
  });
  ...
  describe('getNearestSelectionRange()', () => {
    ...
    test('should return null if there is no valid range', '[]<emptyBlock></emptyBlock>', 'both', null);
  ...
  describe('removeDisallowedAttributes()', () => {
    ...
    it('should filter out disallowed attributes from given nodes', () => {
      ...
      model.change(writer => {
        ...
        expect(getData(model, { withoutSelection: true })).to.equal('<$text a="1">foo</$text><image b="1"></image>');
      });
    });
    ...
    it('should filter out disallowed attributes from empty element', () => {
      ...
      model.change(writer => {
        ...
        expect(getData(model, { withoutSelection: true })).to.equal('<div a="1"></div>');
      });
    });
    ...
    it('should filter out disallowed attributes from parent node and all descendants nodes', () => {
      ...
      model.change(writer => {
        ...
        expect(getData(model, { withoutSelection: true })).to.equal('<div a="1"><$text b="1">foo</$text></div>');
      });
    });
    ...
    it('should do not filter out sibling nodes', () => {
      ...
      model.change(writer => {
        ...
        expect(getData(model, { withoutSelection: true })).to.equal(
          '<div><$text a="1">foo</$text>bar<$text a="1">biz</$text></div>'
        );
      });
    });
  });
  ...
  describe('definitions compilation', () => {
    ...
    describe('mix of allowContentOf and allowWhere', () => {
      ...
      it(
        'passes $root>paragraph and $root2>paragraph – where $root2 inherits content of $root' +
    ...
    beforeEach(() => {
      ...
      root1 = new Element('$root', null, [
        ...
        new Element('blockQuote', null, [
          new Element('paragraph', null, 'foo'),
          new Element('listItem', { listType: 'x', listIndent: 0 }, 'foo'),
          new Element('image', null, [new Element('caption', null, 'foo')]),
        ]),
        new Element('image', null, [new Element('caption', null, 'foo')]),
      ]);
      r1p1 = root1.getChild(0);
  ...
  beforeEach(() => {
    root = new Element('$root', null, [
      new Element('blockQuote', { foo: 1 }, [
        new Element('paragraph', { align: 'left' }, [new Text('foo', { bold: true, italic: true })]),
      ]),
    ]);
  });
