  ...
  describe('getSelectedContent', () => {
    ...
    describe('in blocks', () => {
      ...
      it('gets block image', () => {
        setData(
          model,
          '<paragraph>a</paragraph>[<blockImage><caption>Foo</caption></blockImage>]<paragraph>b</paragraph>'
        );
      ...
      it('gets content when multiple text items needs to be removed from the right excess', () => {
        ...
        const content = stringify(getSelectedContent(model, doc.selection));
        expect(content).to.equal('<paragraph>b</paragraph><paragraph>c</paragraph>');
      });
      ...
      it('gets content when multiple text items needs to be removed from the left excess', () => {
        ...
        const content = stringify(getSelectedContent(model, doc.selection));
        expect(content).to.equal('<paragraph>d</paragraph><paragraph>e</paragraph>');
      });
    });
    ...
    describe('in blocks (deeply nested)', () => {
      ...
      it('gets content when left end nested deeper 2', () => {
        ...
        expect(content).to.equal(
          '<quote><paragraph>bc</paragraph><heading1>x</heading1></quote><paragraph>de</paragraph>'
        );
      });
      ...
      it('gets content when both ends nested deeper than the middle element', () => {
        ...
        const content = stringify(getSelectedContent(model, doc.selection));
        expect(content).to.equal(
          '<quote><heading1>bc</heading1></quote><heading1>x</heading1><quote><heading1>de</heading1></quote>'
        );
      });
      ...
      it('ensures that elements are retrieved by indexes instead of offsets', () => {
        ...
        setData(model, 'foo' + '<quote>' + '<paragraph>' + 'b[ar' + '</paragraph>' + 'bo]m' + '</quote>');

        const content = stringify(getSelectedContent(model, doc.selection));
        expect(content).to.equal('<paragraph>ar</paragraph>bo');
      });
    });
  });
});
