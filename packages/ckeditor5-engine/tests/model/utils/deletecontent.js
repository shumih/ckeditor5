...
describe('DataController utils', () => {
  ...
  describe('deleteContent', () => {
    ...
    describe('in simple scenarios', () => {
      ...
      test('does nothing on collapsed selection', 'f[]oo', 'f[]oo');

      test('deletes single character', 'f[o]o', 'f[]o');
      ...
      test('deletes an element', 'x[<image></image>]y', 'x[]y');

      test('deletes a bunch of nodes', 'w[x<image></image>y]z', 'w[]z');

      test('does not break things when option.merge passed', 'w[x<image></image>y]z', 'w[]z');
    });
    ...
    describe('with text attributes', () => {
      ...
      it('leaves selection attributes when text contains them', () => {
        setData(model, '<paragraph>x<$text bold="true">a[foo]b</$text>y</paragraph>', {
          selectionAttributes: {
            bold: true,
          },
        });
      ...
      describe('with nested elements', () => {
        ...
        it('merges elements when deep nested (3rd level)', () => {
          ...
          root._appendChild(
            new Element('pparent', null, [
              'x',
              new Element('paragraph', null, ['x', new Element('pchild', null, 'foo')]),
            ])
          );

          root._appendChild(
            new Element('pparent', null, [
              new Element('paragraph', null, [new Element('pchild', null, 'bar'), 'y']),
              'y',
            ])
          );
          ...
          expect(getData(model)).to.equal('<pparent>x<paragraph>x<pchild>fo[]ar</pchild>y</paragraph>y</pparent>');
        });
        ...
        it('merges elements when left end deep nested (3rd level)', () => {
          ...
          root._appendChild(
            new Element('pparent', null, [
              'x',
              new Element('paragraph', null, ['foo', new Element('pchild', null, 'bar')]),
            ])
          );

          root._appendChild(new Element('paragraph', null, 'bom'));
          ...
          expect(getData(model)).to.equal('<pparent>x<paragraph>foo<pchild>ba[]om</pchild></paragraph></pparent>');
        });
        ...
        it('merges elements when right end deep nested (3rd level)', () => {
          ...
          root._appendChild(new Element('paragraph', null, 'foo'));

          root._appendChild(
            new Element('pparent', null, [new Element('paragraph', null, [new Element('pchild', null, 'bar')])])
          );
          ...
          expect(getData(model)).to.equal('<paragraph>fo[]</paragraph>');
        });
      });
    ...
    describe('in element selections scenarios', () => {
      ...
      it('deletes two inline elements', () => {
        ...
        setData(model, 'x[<image></image><image></image>]z', { rootName: 'paragraphRoot' });

        deleteContent(model, doc.selection);

        expect(getData(model, { rootName: 'paragraphRoot' })).to.equal('x[]z');
      });

      it('moves the (custom) selection to the nearest paragraph', () => {
        setData(model, '<paragraph>[x]</paragraph><paragraph>yyy</paragraph><paragraph>z</paragraph>', {
          rootName: 'bodyRoot',
        });
        ...
        const selection = new Selection([new Range(new Position(root, [1]), new Position(root, [2]))]);

        deleteContent(model, selection);

        expect(getData(model, { rootName: 'bodyRoot' })).to.equal(
          '<paragraph>[x]</paragraph><paragraph></paragraph><paragraph>z</paragraph>'
        );

        expect(stringify(root, selection)).to.equal(
          '<$root><paragraph>x</paragraph><paragraph>[]</paragraph><paragraph>z</paragraph></$root>'
        );
      });

      it('creates a paragraph when text is not allowed (block widget selected)', () => {
        setData(model, '<paragraph>x</paragraph>[<blockWidget></blockWidget>]<paragraph>z</paragraph>', {
          rootName: 'bodyRoot',
        });
      ...
      it('creates paragraph when text is not allowed (heading selected)', () => {
        setData(model, '<paragraph>x</paragraph><heading1>yyy</heading1><paragraph>z</paragraph>', {
          rootName: 'bodyRoot',
        });

        // [<heading1>yyy</heading1>]
        const range = new Range(new Position(doc.getRoot('bodyRoot'), [1]), new Position(doc.getRoot('bodyRoot'), [2]));

        deleteContent(model, new Selection(range));

        expect(getData(model, { rootName: 'bodyRoot', withoutSelection: true })).to.equal(
          '<paragraph>x</paragraph><paragraph></paragraph><paragraph>z</paragraph>'
        );
      });
      ...
      it('creates paragraph when text is not allowed (two blocks selected)', () => {
        ...
        const range = new Range(new Position(doc.getRoot('bodyRoot'), [1]), new Position(doc.getRoot('bodyRoot'), [3]));
      ...
      it('creates paragraph when text is not allowed (all content selected)', () => {
        setData(model, '[<heading1>x</heading1><paragraph>z</paragraph>]', { rootName: 'bodyRoot' });
      ...
      it('does not create a paragraph when it is not allowed', () => {
        setData(model, '<blockWidget></blockWidget>[<blockWidget></blockWidget>]<blockWidget></blockWidget>', {
          rootName: 'restrictedRoot',
        });

        deleteContent(model, doc.selection);

        expect(getData(model, { rootName: 'restrictedRoot' })).to.equal(
      ...
      it('does not create a paragraph when doNotAutoparagraph option is set to true', () => {
        ...
        expect(getData(model, { rootName: 'bodyRoot' })).to.equal('<paragraph>x[]</paragraph><paragraph>z</paragraph>');
      });

      it('does not create a paragraph when after deletion there is no valid selection range (empty root)', () => {
        setData(model, '[<blockWidget></blockWidget>]', { rootName: 'bodyRoot' });
    ...
    describe('integration with inline limit elements', () => {
      ...
      test('should delete whole inline limit element', 'x[<inlineLimit>foo bar</inlineLimit>]x', 'x[]x');
    ...
    describe('integration with block limit elements', () => {
      ...
      it('should proper merge two elements which are inside limit element', () => {
        ...
        expect(getData(model)).to.equal(
          '<blockLimit>' + '<blockQuote>' + '<paragraph>Foo[]Bar</paragraph>' + '</blockQuote>' + '</blockLimit>'
        );
      });
    ...
    describe('should leave a paragraph if the entire content was selected', () => {
      ...
      test('but not if only one block was selected', '<heading1>[xx]</heading1>', '<heading1>[]</heading1>');
