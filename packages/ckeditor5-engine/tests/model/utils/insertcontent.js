  ...
  describe('insertContent', () => {
    ...
    describe('in simple scenarios', () => {
      ...
      it('inserts one text node with attribute into text with a different attribute', () => {
        ...
        expect(getData(model)).to.equal(
          '<$text bold="true">f</$text><$text italic="true">xyz[]</$text><$text bold="true">oo</$text>'
        );
    ...
    describe('in blocks', () => {
      ...
      it('not insert autoparagraph when paragraph is disallowed at the current position', () => {
        ...
        const content = new DocumentFragment([new Element('heading1', [], [new Text('bar')]), new Text('biz')]);
      ...
      describe('block to block handling', () => {
        ...
        it('should not merge a paragraph wrapped in blockQuote with list item (checking right merge)', () => {
          ...
          const affectedRange = insertHelper(
            '<heading1>yyy</heading1><blockQuote><paragraph>xxx</paragraph></blockQuote>'
          );
        ...
        it('should handle bQ+p over bQ+p insertion', () => {
          ...
          const affectedRange = insertHelper(
            '<blockQuote><paragraph>xxx</paragraph></blockQuote><paragraph>yyy</paragraph>'
          );
      ...
      describe('mixed content to block', () => {
        ...
        it('inserts object + text (at the beginning)', () => {
          ...
          expect(getData(model)).to.equal('<blockWidget></blockWidget><paragraph>xxx[]foo</paragraph>');

          expect(stringify(root, affectedRange)).to.equal('[<blockWidget></blockWidget><paragraph>xxx]foo</paragraph>');
        });
        ...
        it('inserts text + object (at the end)', () => {
          ...
          expect(getData(model)).to.equal('<paragraph>fooxxx</paragraph>[<blockWidget></blockWidget>]');

          expect(stringify(root, affectedRange)).to.equal('<paragraph>foo[xxx</paragraph><blockWidget></blockWidget>]');
        });
      });
    ...
    describe('filtering out', () => {
      ...
      it('filters out disallowed elements and leaves out the paragraphs', () => {
        ...
        expect(stringify(root, affectedRange)).to.equal(
          '<paragraph>f[xxx</paragraph><paragraph>yyy</paragraph><paragraph>zzz]oo</paragraph>'
        );
      });
      ...
      it('filters out disallowed attributes when inserting nested elements', () => {
        ...
        expect(stringify(root, affectedRange)).to.equal(
          '<element>[<table><td>f<$text b="1">o</$text>o</td></table>]</element>'
        );
      });
      ...
      it('filters out disallowed attributes from nested nodes when merging', () => {
        ...
        expect(stringify(root, affectedRange)).to.equal(
          '<paragraph>f[x<element>b<$text b="1">a</$text>r</element>x]oo</paragraph>'
        );
      });
      ...
      it('filters out disallowed attributes when autoparagraphing', () => {
        ...
        expect(getData(model)).to.equal(
          '<paragraph>fxxx</paragraph><paragraph><$text b="1">yyy[]</$text>oo</paragraph>'
        );

        expect(stringify(root, affectedRange)).to.equal(
          '<paragraph>f[xxx</paragraph><paragraph><$text b="1">yyy</$text>]oo</paragraph>'
        );
      });
    });
  });
