...
describe('DowncastWriter', () => {
  describe('insert()', () => {
    ...
    it('should return collapsed range in insertion position when using empty array', () => {
      testInsert('<container:p>foo{}bar</container:p>', [], '<container:p>foo{}bar</container:p>');
    });

    it('should insert text into another text node #1', () => {
      testInsert('<container:p>foo{}bar</container:p>', ['baz'], '<container:p>foo{baz}bar</container:p>');
    });

    it('should insert text into another text node #2', () => {
      testInsert('<container:p>foobar{}</container:p>', ['baz'], '<container:p>foobar{baz]</container:p>');
    });

    it('should insert text into another text node #3', () => {
      testInsert('<container:p>{}foobar</container:p>', ['baz'], '<container:p>[baz}foobar</container:p>');
    });
    ...
    it('should insert text into in document fragment', () => {
      testInsert('foo{}bar', ['baz'], 'foo{baz}bar');
    });
    ...
    it('should insert unicode text into unicode text', () => {
      testInsert('நி{}க்கு', ['லை'], 'நி{லை}க்கு');
    });
    ...
    it('should throw when inserting Element', () => {
      ...
      expectToThrowCKEditorError(
        ...
        'view-writer-insert-invalid-node',
        document
      );
    });
