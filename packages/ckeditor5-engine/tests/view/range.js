...
describe('Range', () => {
  ...
  describe('getEnlarged', () => {
    it('case 1', () => {
      expect(enlarge('<p>f<b>{oo}</b></p><p>bar</p>')).to.equal('<p>f[<b>oo</b>]</p><p>bar</p>');
    });

    it('case 2', () => {
      expect(enlarge('<p>f{oo}bar</p>')).to.equal('<p>f{oo}bar</p>');
    });

    it('case 3', () => {
      expect(enlarge('<p>f<span></span>{oo}<span></span>bar</p>')).to.equal(
        '<p>f[<span></span>oo<span></span>]bar</p>'
      );
    });

    it('case 4', () => {
      expect(enlarge('<p>f<img></img>{oo}<img></img>bar</p>')).to.equal('<p>f<img></img>[oo]<img></img>bar</p>');
    });

    it('case 5', () => {
      expect(enlarge('<p><b>f</b>{oo}<b><span></span>bar</b></p>')).to.equal(
        '<p><b>f[</b>oo<b><span></span>]bar</b></p>'
      );
    });

    it('case6', () => {
      expect(enlarge('<p>foo</p><p>[bar]</p><p>bom</p>')).to.equal('<p>foo</p><p>[bar]</p><p>bom</p>');
    });
  ...
  describe('getTrimmed', () => {
    it('case 1', () => {
      expect(trim('<p>f[<b>oo</b>]</p><p>bar</p>')).to.equal('<p>f<b>{oo}</b></p><p>bar</p>');
    });

    it('case 2', () => {
      expect(trim('<p>f{oo}bar</p>')).to.equal('<p>f{oo}bar</p>');
    });

    it('case 3', () => {
      expect(trim('<p>f[<span></span>oo<span></span>]bar</p>')).to.equal('<p>f<span></span>{oo}<span></span>bar</p>');
    });

    it('case 4', () => {
      expect(trim('<p>f<img></img>[oo]<img></img>bar</p>')).to.equal('<p>f<img></img>{oo}<img></img>bar</p>');
    });

    it('case 5', () => {
      expect(trim('<p><b>f[</b>oo<b><span></span>]bar</b></p>')).to.equal('<p><b>f</b>{oo}<b><span></span>bar</b></p>');
    });

    it('case 6', () => {
      expect(trim('<p>foo[</p><p>bar</p><p>]bom</p>')).to.equal('<p>foo[</p><p>bar</p><p>]bom</p>');
    });

    it('case 7', () => {
      expect(trim('<p>foo[<b><img></img></b>]bom</p>')).to.equal('<p>foo<b>[<img></img>]</b>bom</p>');
    });
    ...
    it('case 8', () => {
      expect(trim('<p>[<b></b>]</p>')).to.equal('<p><b></b>[]</p>');
    });

    // As above.
    it('case 9', () => {
      expect(trim('<p><b></b>[<b></b>]<b></b></p>')).to.equal('<p><b></b><b></b><b></b>[]</p>');
    });

    // As above.
    it('case 10', () => {
      expect(trim('<p>[<b></b><b></b>]</p>')).to.equal('<p><b></b><b></b>[]</p>');
    });
