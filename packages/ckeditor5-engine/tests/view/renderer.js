...
describe('Renderer', () => {
  ...
  describe('render', () => {
    ...
    it('should add and remove inline filler in case <p>[]<b>foo</b></p>', () => {
      ...
      // Step 3: <p><b>{}foo</b></p>
      selection._setTo(
        ViewRange._createFromParentsAndOffsets(viewP.getChild(0).getChild(0), 0, viewP.getChild(0).getChild(0), 0)
      );
    ...
    it('should add and remove inline filler in case <p><b>foo</b>[]</p>', () => {
      ...
      // Step 3: <p><b>foo{}</b></p>
      selection._setTo(
        ViewRange._createFromParentsAndOffsets(viewP.getChild(0).getChild(0), 3, viewP.getChild(0).getChild(0), 3)
      );
    ...
    it('should remove filler from a modified DOM in case <p>bar<b>foo</b>[]</p>', () => {
      ...
      const { view: viewP, selection: newSelection } = parse(
        '<container:p>bar<attribute:b>foo</attribute:b>[]</container:p>'
      );
      viewRoot._appendChild(viewP);
      selection._setTo(newSelection);
    ...
    it('should not break when selection is moved to a new element, when filler exists', () => {
      ...
      const { view: viewP, selection: newSelection } = parse(
        '<container:p>bar<attribute:b>[]</attribute:b></container:p>'
      );
      viewRoot._appendChild(viewP);
      selection._setTo(newSelection);
    ...
    it('should remove inline filler if selection is before a view element not bound to dom', () => {
      ...
      const { view: viewP, selection: newSelection } = parse(
        '<container:p>bar<attribute:b>abc</attribute:b>[]</container:p>'
      );
      viewRoot._appendChild(viewP);
      selection._setTo(newSelection);
    ...
    it('should not force–refresh the selection in non–Gecko browsers after a soft break', () => {
      ...
      // <p>foo<br/>[]</p>
      const { view: viewP, selection: newSelection } = parse(
        '<container:p>' + 'foo[]' + '<empty:br></empty:br>[]' + '</container:p>'
      );
    ...
    it('should force–refresh the selection in Gecko browsers after a soft break to nudge the caret', () => {
      ...
      // <p>foo<b>bar</b></p><p>foo<br/>[]</p>
      selection._setTo([new ViewRange(new ViewPosition(viewP, 2), new ViewPosition(viewP, 2))]);
    ...
    describe('fake selection', () => {
      beforeEach(() => {
        const { view: viewP, selection: newSelection } = parse('<container:p>[foo bar]</container:p>');
        viewRoot._appendChild(viewP);
        selection._setTo(newSelection);
        renderer.markToSync('children', viewRoot);
        renderer.render();
      ...
      function assertDomSelectionContents(domSelection, expectedContainer, expectedText) {
        ...
        expect(domSelectionContainer == expectedContainer.firstChild || domSelectionContainer == expectedContainer).to
          .be.true;
      }
    });
    ...
    describe('optimal rendering – reusing existing nodes', () => {
      ...
      it('should render single replacement in p group', () => {
        const content =
          '' + '<container:p>1</container:p>' + '<container:p>2</container:p>' + '<container:p>3</container:p>';
      ...
      it('should render multiple continuous replacement in p group', () => {
        ...
        viewRoot._insertChild(
          0,
          parse('<container:p>6<attribute:i>A</attribute:i></container:p><container:p>7</container:p>')
        );
      ...
      it('should not rerender DOM when view replaced with the same structure', () => {
        ...
        expect(domRoot.innerHTML).to.equal(
          '<h2>He<i>ading 1</i></h2><p>Ph <strong>Bold</strong>' +
        ...
        expect(mappings.get(domPABI)).to.equal(
          viewP
            .getChild(2)
            .getChild(0)
            .getChild(1)
        ...
        expect(mappings.get(domQULB)).to.equal(
          viewQ
            .getChild(0)
            .getChild(0)
            .getChild(1)
      ...
      it('should handle complex view duplication', () => {
        ...
        expect(domMappings.get(domLI1)).to.equal(
          viewRoot
            .getChild(0)
            .getChild(0)
            .getChild(0)
        );
        expect(domMappings.get(domLI2)).to.equal(
          viewRoot
            .getChild(0)
            .getChild(0)
            .getChild(1)
        );
        expect(domMappings.get(domLI3)).to.equal(
          viewRoot
            .getChild(0)
            .getChild(0)
            .getChild(2)
      ...
      it('should handle classes change in replaced elements', () => {
        ...
        expect(normalizeHtml(domRoot.innerHTML)).to.equal(
          normalizeHtml('<ol><li class="foo1 bar2 baz3">Item 1</li></ol><p><i class="i1 i2">Bar</i>Baz</p>')
        );
        ...
        expect(normalizeHtml(domRoot.innerHTML)).to.equal(
          normalizeHtml('<ol><li class="bar2 baz4 bax5">Item 1</li></ol><p class="p1 p2"><i>Foo</i></p>')
        );
      });
      ...
      it('should handle styles change in replaced elements', () => {
        ...
        viewLI2._insertChild(
          0,
          parse(
            'Ba1 <attribute:i style="color:#000;border-width:1px;">Ba3 ' +
      ...
      it('should handle uiElement rendering', () => {
        ...
        expect(normalizeHtml(domRoot.innerHTML)).to.equal(
          normalizeHtml('<p>Foo <span><span id="id1"><b>UI1</b></span></span>Bar</p>')
        );
        ...
        expect(normalizeHtml(domRoot.innerHTML)).to.equal(
          normalizeHtml('<p>Foo<span><span id="id2"><b>UI2</b></span></span> Bar</p>')
        );
      });
      ...
      it('should correctly handle moving a DOM element when rendering children (more complex case)', () => {
        viewRoot._insertChild(
          0,
          parse(
            '1<attribute:span>2</attribute:span><attribute:span>3</attribute:span>4<attribute:span>5</attribute:span>'
          )
        );
        renderer.markToSync('children', viewRoot);
        renderer.render();
        ...
        expect(domRoot.innerHTML).to.equal(
          '<span>2</span><strong>6</strong><span>5</span>1<span>3</span><strong>7</strong>4'
        );
      });
    });
    ...
    describe('optimal (minimal) rendering – minimal children changes', () => {
      ...
      it('should add only one child (at the beginning)', () => {
        ...
        expect(getMutationStats(observer.takeRecords())).to.deep.equal(['added: 1, removed: 0']);
      });
      ...
      it('should not touch the FSC when rendering children', () => {
        ...
        expect(getMutationStats(observer.takeRecords())).to.deep.equal(['added: 0, removed: 1']);
      });
      ...
      it('should not incorrectly remove element which is not a FSC when rendering children', () => {
        ...
        viewRoot._appendChild(
          parse('<container:div><container:p>1</container:p><container:p>2</container:p></container:div>')
        );
