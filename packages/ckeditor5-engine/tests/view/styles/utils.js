...
describe('Styles utils', () => {
  describe('isColor()', () => {
    ...
    it('returns false for wrong rgba() color', () => {
      testValues(['rgba(1,2,3,0.7', 'rgba((1,2,3,0.7', 'rgba(1,a,3,0.7)', 'rgba(1,2,3,*)'], value => !isColor(value));
    });
  ...
  describe('isLength()', () => {
    it('returns true for various units', () => {
      testValues(
        ['1px', '1cm', '1mm', '1in', '1pc', '1pt', '1ch', '1em', '1ex', '1rem', '1vh', '1vw', '1vmin', '1vmax'],
        isLength
      );
    });

    it('returns true for various values notation', () => {
      testValues(['0', '1px', '1000px', '1.1px', '345.457px', '.457px'], isLength);
    });
  });
  ...
  describe('getBoxSidesShorthandValue()', () => {
    ...
    it('should output three values if bottom is different then top', () => {
      expect(getBoxSidesShorthandValue({ top: 'foo', right: 'foo', bottom: 'bar', left: 'foo' })).to.equal(
        'foo foo bar'
      );
    });

    it('should output four values if left is different then right', () => {
      expect(getBoxSidesShorthandValue({ top: 'foo', right: 'foo', bottom: 'foo', left: 'bar' })).to.equal(
        'foo foo foo bar'
      );
    });
  });
  ...
  describe('getParts()', () => {
    ...
    it('should split string to separate values when value contain grouping parens', () => {
      expect(getShorthandValues('foo bar(1, 3, 5) url("example.com:foo/bar?q=b")')).to.deep.equal([
        'foo',
        'bar(1, 3, 5)',
        'url("example.com:foo/bar?q=b")',
      ]);
    });
  });
