...
describe('DomConverter – whitespace handling – integration', () => {
  ...
  describe('normalizing whitespaces around block boundaries (#822)', () => {
    ...
    it('nbsp between blocks is not ignored (different blocks)', () => {
      ...
      expect(getData(editor.model, { withoutSelection: true })).to.equal(
        '<block>foo</block>' + '<paragraph> </paragraph>' + '<paragraph>bar</paragraph>'
      );

      expect(editor.getData()).to.equal('<block>foo</block>' + '<p>&nbsp;</p>' + '<p>bar</p>');
    });
    ...
    it('single nbsp inside blocks are ignored', () => {
      ...
      expect(getData(editor.model, { withoutSelection: true })).to.equal('<paragraph></paragraph>');
    ...
    it('nbsp between inline elements is not ignored', () => {
      ...
      expect(getData(editor.model, { withoutSelection: true })).to.equal(
        '<paragraph><$text bold="true">foo</$text>\u00A0<$text bold="true">bar</$text></paragraph>'
      );
  ...
  describe('whitespaces around <br>s', () => {
    beforeEach(() => {
      return VirtualTestEditor.create({ plugins: [Paragraph, ShiftEnter] }).then(newEditor => {
        editor = newEditor;
      });
    });
