...
describe('model test utils', () => {
  ...
  describe('getData', () => {
    ...
    describe('markers', () => {
      it('should stringify collapsed marker', () => {
        ...
        expect(getData(model, { convertMarkers: true, withoutSelection: true })).to.equal(
          '<foo:start></foo:start><paragraph>bar</paragraph>'
        );
      });
      ...
      it('should stringify non-collapsed marker', () => {
        ...
        const markerRange = new Range(
          Position._createAt(document.getRoot(), 0),
          Position._createAt(document.getRoot(), 1)
        );

        model.markers._set('foo', markerRange);

        expect(getData(model, { convertMarkers: true, withoutSelection: true })).to.equal(
          '<foo:start></foo:start><paragraph>bar</paragraph><foo:end></foo:end>'
        );
      });
    });
  });
  ...
  describe('stringify', () => {
    ...
    it('should stringify element', () => {
      const element = new Element('a', null, [new Element('b', null, new Text('btext')), new Text('atext')]);

      expect(stringify(element)).to.equal('<a><b>btext</b>atext</a>');
    });

    it('should stringify document fragment', () => {
      const fragment = new DocumentFragment([new Element('b', null, new Text('btext')), new Text('atext')]);
    ...
    it('writes elements and texts', () => {
      root._appendChild([
        ...
        new Element('b', null, [new Element('c1'), new Text('ctext'), new Element('c2')]),
        new Element('d'),
      ]);

      expect(stringify(root)).to.equal('<a>atext</a><b><c1></c1>ctext<c2></c2></b><d></d>');
    });
    ...
    it('writes text attributes', () => {
      root._appendChild([
        ...
        new Element('a', null, [new Text('pom', { underline: true, bold: true })]),
      ]);
    ...
    describe('selection', () => {
      ...
      beforeEach(() => {
        ...
        root._appendChild([elA, new Text('foo'), new Text('bar', { bold: true }), elB]);
      });
      ...
      it('writes selection collapsed in an element', () => {
        ...
        expect(stringify(root, selection)).to.equal('[]<a></a>foo<$text bold="true">bar</$text><b></b>');
      });
      ...
      it('writes selection collapsed in a text', () => {
        ...
        expect(stringify(root, selection)).to.equal('<a></a>fo[]o<$text bold="true">bar</$text><b></b>');
      });
      ...
      it('writes selection collapsed selection in a text with attributes', () => {
        ...
        expect(stringify(root, selection)).to.equal('<a></a>foo<$text bold="true">b[]ar</$text><b></b>');
      });
      ...
      it('writes flat selection within text', () => {
        ...
        expect(stringify(root, selection)).to.equal('<a></a>f[o]o<$text bold="true">bar</$text><b></b>');
      });
      ...
      it('uses range and coverts it to selection', () => {
        ...
        expect(stringify(root, range)).to.equal('<a>[</a>foo<$text bold="true">bar</$text><b>]</b>');
      });

      it('uses position and converts it to collapsed selection', () => {
        const position = new Position(root, [0]);

        expect(stringify(root, position)).to.equal('[]<a></a>foo<$text bold="true">bar</$text><b></b>');
      });
    });
  });
