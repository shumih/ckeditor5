...
describe('OperationReplayer', () => {
  ...
  describe('applyAllOperations()', () => {
    it('should apply all operations on the document', () => {
      ...
      const stringifiedOperations = [getFirstOperation(), getSecondOperation()].map(d => JSON.stringify(d)).join('---');
  ...
  describe('play()', () => {
    ...
    it('should correctly handle errors coming from the engine', () => {
      ...
      const stringifiedOperations = [getFirstOperation(), invalidOperation].map(d => JSON.stringify(d)).join('---');

      const operationReplayer = new OperationReplayer(model, '---', stringifiedOperations);

      return operationReplayer.play(1).then(
        () => {
