...
describe('view test utils', () => {
  describe('getData, setData', () => {
    ...
    describe('getData', () => {
      it('should use stringify method', () => {
        ...
        expect(stringifyOptions)
          .to.have.property('showType')
          .that.equals(false);
        expect(stringifyOptions)
          .to.have.property('showPriority')
          .that.equals(false);
        expect(stringifyOptions)
          .to.have.property('ignoreRoot')
          .that.equals(true);
        expect(stringifyOptions)
          .to.have.property('renderUIElements')
          .that.equals(false);
  ...
  describe('stringify', () => {
    ...
    it('should write selection ranges inside text represented by `[` and `]` characters', () => {
      ...
      const range = Range._createFromParentsAndOffsets(text1, 1, text1, 5);
      const selection = new DocumentSelection([range]);
      expect(stringify(p, selection, { sameSelectionCharacters: true })).to.equal(
        '<p><b>f[ooba]r</b><b>bazqux</b></p>'
      );
    });
    ...
    it('should write elements types as namespaces when needed', () => {
      ...
      expect(stringify(p, null, { showType: true })).to.equal(
        '<container:p><attribute:b>foobar</attribute:b></container:p>'
      );
    });
    ...
    it('should write elements priorities when needed', () => {
      ...
      expect(stringify(p, null, { showPriority: true })).to.equal('<p><b view-priority="10">foobar</b></p>');
    });
    ...
    it('should write elements id when needed', () => {
      ...
      expect(stringify(p, null, { showAttributeElementId: true })).to.equal('<p><span view-id="foo">foobar</span></p>');
    });
    ...
    it('should stringify EmptyElement', () => {
      ...
      expect(stringify(p, null, { showType: true })).to.equal('<container:p><empty:img></empty:img></container:p>');
    });

    it('should stringify UIElement', () => {
      const span = new UIElement(viewDocument, 'span');
      const p = new ContainerElement(viewDocument, 'p', null, span);
      expect(stringify(p, null, { showType: true })).to.equal('<container:p><ui:span></ui:span></container:p>');
    });
    ...
    it('should stringify UIElement, (renderUIElements=true)', () => {
      ...
      const p = new ContainerElement(viewDocument, 'p', null, span);
      expect(stringify(p, null, { showType: true, renderUIElements: true })).to.equal(
        '<container:p><ui:span><b>foo</b></ui:span></container:p>'
      );
    });
