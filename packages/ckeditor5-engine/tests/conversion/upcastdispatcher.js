...
describe('UpcastDispatcher', () => {
  ...
  describe('constructor()', () => {
    it('should create UpcastDispatcher with passed api', () => {
      ...
      expect(dispatcher.conversionApi)
        .to.have.property('convertItem')
        .that.is.instanceof(Function);
      expect(dispatcher.conversionApi)
        .to.have.property('convertChildren')
        .that.is.instanceof(Function);
      expect(dispatcher.conversionApi)
        .to.have.property('splitToAllowedParent')
        .that.is.instanceof(Function);
    });
  ...
  describe('convert()', () => {
    ...
    it('should remove empty elements that was created as a result of split', () => {
      const viewElement = new ViewElement(viewDocument, 'div', null, [
        new ViewElement(viewDocument, 'p', null, [new ViewElement(viewDocument, 'img')]),
      ]);
    ...
    it('should extract temporary markers elements from converter element and create static markers list', () => {
      ...
      expect(marker2.start.path)
        .to.deep.equal(marker2.end.path)
        .to.deep.equal([3]);
    });
    ...
    it('should convert according to given context', () => {
      ...
      // Position as context.
      const fragment = new ModelDocumentFragment([
        new ModelElement('first', { foo: 'bar' }, [new ModelElement('second', null)]),
      ]);
  ...
  describe('conversionApi', () => {
    ...
    describe('convertChildren()', () => {
      it(
        'should fire conversion for all children of passed element and return conversion results ' +
    ...
    describe('splitToAllowedParent()', () => {
      ...
      it('should return null if element is not allowed in position and any of ancestors but is allowed in context tree', () => {
        ...
        model.change(writer =>
          dispatcher.convert(new ViewDocumentFragment(viewDocument), writer, ['$root', 'paragraph'])
        );
        sinon.assert.calledOnce(spy);
      });
    });
