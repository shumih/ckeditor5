...
} from '../../src/conversion/upcasthelpers';
...
describe('UpcastHelpers', () => {
  ...
  describe('.elementToElement()', () => {
    ...
    it('config.view is an object', () => {
      ...
      expectResult(
        new ViewContainerElement(viewDocument, 'p', { class: 'fancy' }),
  ...
  describe('.elementToAttribute()', () => {
    ...
    it('should not set an attribute if it is not allowed by schema', () => {
      ...
      expectResult(new ViewAttributeElement(viewDocument, 'em', null, new ViewText(viewDocument, 'foo')), 'foo');
    });
  ...
  describe('.attributeToAttribute()', () => {
    ...
    it('config.view has only key and name set', () => {
      ...
      upcastHelpers.attributeToAttribute({
        view: { name: 'img', key: 'src' },
        model: { name: 'image', key: 'source' },
      });
    ...
    it('model attribute value is a string', () => {
      ...
      expectResult(new ViewContainerElement(viewDocument, 'img', { class: 'styled-xxx' }), '<image></image>');

      expectResult(new ViewContainerElement(viewDocument, 'p', { class: 'styled-dark' }), '<paragraph></paragraph>');
    });
    ...
    it('should not set an attribute if it is not allowed by schema', () => {
      ...
      expectResult(new ViewAttributeElement(viewDocument, 'img', { src: 'foo.jpg' }), '<image></image>');
    });
    ...
    it('should not do anything if returned model attribute is null', () => {
      ...
      expectResult(new ViewAttributeElement(viewDocument, 'img', { class: 'styled' }), '<image styled="true"></image>');
    });
...
describe('upcast-converters', () => {
  ...
  describe('convertSelectionChange()', () => {
    ...
    it('should convert collapsed selection', () => {
      ...
      viewSelection.setTo(
        ViewRange._createFromParentsAndOffsets(viewRoot.getChild(0).getChild(0), 1, viewRoot.getChild(0).getChild(0), 1)
      );
    ...
    it('should convert multi ranges selection', () => {
      const viewSelection = new ViewSelection([
        ...
        ViewRange._createFromParentsAndOffsets(
          viewRoot.getChild(1).getChild(0),
          1,
          viewRoot.getChild(1).getChild(0),
          2
        ),
