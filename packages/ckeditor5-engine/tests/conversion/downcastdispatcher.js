...
describe('DowncastDispatcher', () => {
  ...
  describe('convertChanges', () => {
    ...
    it('should call convertMarkerAdd when markers are added', () => {
      ...
      differStub.getMarkersToAdd = () => [{ name: 'foo', range: fooRange }, { name: 'bar', range: barRange }];
    ...
    it('should call convertMarkerRemove when markers are removed', () => {
      ...
      differStub.getMarkersToRemove = () => [{ name: 'foo', range: fooRange }, { name: 'bar', range: barRange }];
  ...
  describe('convertInsert', () => {
    it('should fire event with correct parameters for every item in passed range', () => {
      ...
      dispatcher.on('attribute', (evt, data, conversionApi) => {
        ...
        const log =
          'attribute:' + key + ':' + value + ':' + itemId + ':' + data.range.start.path + ':' + data.range.end.path;
  ...
  describe('convertSelection', () => {
    ...
    it('should fire selection event', () => {
      ...
      expect(dispatcher.fire.calledWith('selection', { selection: sinon.match.instanceOf(doc.selection.constructor) }))
        .to.be.true;
    });
  ...
  describe('convertMarkerAdd', () => {
    ...
    it('should not convert marker if it is in graveyard', () => {
      const gyRange = model.createRange(
        model.createPositionAt(doc.graveyard, 0),
        model.createPositionAt(doc.graveyard, 0)
      );
      sinon.spy(dispatcher, 'fire');
