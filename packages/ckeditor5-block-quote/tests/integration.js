...
describe('BlockQuote integration', () => {
  ...
  describe('enter key support', () => {
    ...
    it('escapes block quote if selection is in an empty block in an empty block quote', () => {
      ...
      setModelData(
        model,
        '<paragraph>x</paragraph><blockQuote><paragraph>[]</paragraph></blockQuote><paragraph>x</paragraph>'
      );
  ...
  describe('backspace key support', () => {
    ...
    it('merges paragraph into paragraph in the quote', () => {
      ...
      expect(getModelData(model)).to.equal(
        '<blockQuote><paragraph>a</paragraph><paragraph>b[]c</paragraph></blockQuote>' + '<paragraph>d</paragraph>'
      );
    });
    ...
    it('merges paragraph from a quote into a paragraph before quote', () => {
      ...
      expect(getModelData(model)).to.equal(
        '<paragraph>x[]a</paragraph>' + '<blockQuote><paragraph>b</paragraph></blockQuote>' + '<paragraph>y</paragraph>'
      );
    });
    ...
    it('removes empty quote when merging into another quote', () => {
      ...
      expect(getModelData(model)).to.equal(
        '<paragraph>x</paragraph>' + '<blockQuote><paragraph>a[]</paragraph></blockQuote>' + '<paragraph>y</paragraph>'
      );
    });
    ...
    it('removes empty quote when merging into a paragraph', () => {
      ...
      setModelData(
        model,
        '<paragraph>x</paragraph>' + '<blockQuote><paragraph>[]</paragraph></blockQuote>' + '<paragraph>y</paragraph>'
      );
  ...
  describe('compatibility with images', () => {
    it('quotes a simple image', () => {
      ...
      return ClassicTestEditor.create(element, {
        ...
        setModelData(
          editor.model,
          '<paragraph>fo[o</paragraph>' + '<image src="/assets/sample.png"></image>' + '<paragraph>b]ar</paragraph>'
        );
    ...
    it('wraps paragraph+image', () => {
      setModelData(model, '<paragraph>[foo</paragraph><image><caption>foo</caption></image>]');
    ...
    it('unwraps paragraph+image', () => {
      setModelData(model, '<blockQuote><paragraph>[foo</paragraph><image><caption>foo</caption></image>]</blockQuote>');

      editor.execute('blockQuote');

      expect(getModelData(model)).to.equal('<paragraph>[foo</paragraph><image><caption>foo</caption></image>]');
    });

    it('wraps image+paragraph', () => {
      setModelData(model, '[<image><caption>foo</caption></image><paragraph>foo]</paragraph>');
  ...
  describe('autoparagraphing', () => {
    it('text in block quote in div', () => {
      const data = '<blockquote>' + '<div>foo<strong>bar</strong></div>' + '</blockquote>' + 'xyz';

      editor.setData(data);

      expect(editor.getData()).to.equal(
        '<blockquote>' + '<p>foo<strong>bar</strong></p>' + '</blockquote>' + '<p>xyz</p>'
      );
    });

    it('text directly in block quote', () => {
      const data = '<blockquote>' + 'foo<strong>bar</strong>' + '</blockquote>' + 'xyz';
    ...
    it('text after block quote in div', () => {
      const data = '<blockquote>' + 'foo<strong>bar</strong>' + '</blockquote>' + '<div>xyz</div>';
    ...
    it('text inside block quote in and after div', () => {
      const data = '<blockquote>' + '<div>foo</div><strong>bar</strong>' + '</blockquote>' + 'xyz';

      editor.setData(data);

      expect(editor.getData()).to.equal(
        '<blockquote>' + '<p>foo</p><p><strong>bar</strong></p>' + '</blockquote>' + '<p>xyz</p>'
      );
    });

    it('text inside block quote in div split by heading', () => {
      const data = '<blockquote>' + '<div>foo<h2>bar</h2><strong>baz</strong></div>' + '</blockquote>' + 'xyz';

      editor.setData(data);

      expect(editor.getData()).to.equal(
        '<blockquote>' + '<p>foo</p><h2>bar</h2><p><strong>baz</strong></p>' + '</blockquote>' + '<p>xyz</p>'
      );
    });
  });
});
