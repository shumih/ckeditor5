  ...
  describe('execute()', () => {
    describe('applying quote', () => {
      it('should wrap a single block', () => {
        setModelData(
          model,
          '<paragraph>abc</paragraph>' + '<paragraph>x[]x</paragraph>' + '<paragraph>def</paragraph>'
        );
        ...
        expect(getViewData(editor.editing.view)).to.equal('<p>abc</p><blockquote><p>x{}x</p></blockquote><p>def</p>');
      });

      it('should wrap multiple blocks', () => {
        setModelData(model, '<heading>a[bc</heading>' + '<paragraph>xx</paragraph>' + '<paragraph>de]f</paragraph>');
        ...
        expect(getViewData(editor.editing.view)).to.equal('<blockquote><h>a{bc</h><p>xx</p><p>de}f</p></blockquote>');
      });
      ...
      it('should not merge with a quote preceding the current block', () => {
        setModelData(model, '<blockQuote><paragraph>abc</paragraph></blockQuote>' + '<paragraph>x[]x</paragraph>');

        editor.execute('blockQuote');

        expect(getModelData(model)).to.equal(
          '<blockQuote><paragraph>abc</paragraph></blockQuote>' + '<blockQuote><paragraph>x[]x</paragraph></blockQuote>'
        );

        expect(getViewData(editor.editing.view)).to.equal(
          '<blockquote><p>abc</p></blockquote>' + '<blockquote><p>x{}x</p></blockquote>'
        );
      });

      it('should not merge with a quote following the current block', () => {
        setModelData(model, '<paragraph>x[]x</paragraph>' + '<blockQuote><paragraph>abc</paragraph></blockQuote>');

        editor.execute('blockQuote');

        expect(getModelData(model)).to.equal(
          '<blockQuote><paragraph>x[]x</paragraph></blockQuote>' + '<blockQuote><paragraph>abc</paragraph></blockQuote>'
        );

        expect(getViewData(editor.editing.view)).to.equal(
          '<blockquote><p>x{}x</p></blockquote>' + '<blockquote><p>abc</p></blockquote>'
        );
      });
      ...
      it('should not wrap non-block content', () => {
        setModelData(model, '<paragraph>a[bc</paragraph>' + '<widget>xx</widget>' + '<paragraph>de]f</paragraph>');
      ...
      it('should not wrap a block which can not be in a quote', () => {
        ...
        setModelData(model, '<paragraph>a[bc</paragraph>' + '<fooBlock>xx</fooBlock>' + '<paragraph>de]f</paragraph>');
      ...
      it('should handle forceValue = true param', () => {
        setModelData(
          model,
          '<blockQuote>' + '<paragraph>x[x</paragraph>' + '</blockQuote>' + '<paragraph>d]ef</paragraph>'
        );

        editor.execute('blockQuote', { forceValue: true });

        expect(getModelData(model)).to.equal(
          '<blockQuote>' + '<paragraph>x[x</paragraph>' + '<paragraph>d]ef</paragraph>' + '</blockQuote>'
        );

        expect(getViewData(editor.editing.view)).to.equal('<blockquote><p>x{x</p><p>d}ef</p></blockquote>');
      });
    });
    ...
    describe('removing quote', () => {
      it('should unwrap a single block', () => {
        ...
        expect(getViewData(editor.editing.view)).to.equal('<p>abc</p><p>x{}x</p><p>def</p>');
      });
      ...
      it('should unwrap multiple blocks', () => {
        ...
        expect(getModelData(model)).to.equal(
          '<paragraph>a[bc</paragraph>' + '<paragraph>xx</paragraph>' + '<paragraph>de]f</paragraph>'
        );

        expect(getViewData(editor.editing.view)).to.equal('<p>a{bc</p><p>xx</p><p>de}f</p>');
      });
      ...
      it('should unwrap only the selected blocks - at the end', () => {
        ...
        expect(getViewData(editor.editing.view)).to.equal('<blockquote><p>abc</p></blockquote><p>x{x</p><p>de}f</p>');
      });
      ...
      it('should handle forceValue = false param', () => {
        setModelData(
          model,
          '<paragraph>a[bc</paragraph>' + '<blockQuote>' + '<paragraph>x]x</paragraph>' + '</blockQuote>'
        );

        editor.execute('blockQuote', { forceValue: false });

        // Incorrect selection.
        expect(getModelData(model)).to.equal('<paragraph>a[bc]</paragraph>' + '<paragraph>xx</paragraph>');

        expect(getViewData(editor.editing.view)).to.equal('<p>a{bc}</p><p>xx</p>');
      });
    });
  });
});
