...
describe('Typing – InputCommand integration', () => {
  ...
  describe('InputCommand integration', () => {
    ...
    it('resets the buffer when selection changes (with enter)', () => {
      ...
      expectOutput(
        '<paragraph>Foo1</paragraph><paragraph>Bar2</paragraph><paragraph>Baz[]</paragraph>',
      ...
      expectOutput(
        '<paragraph>Foo1</paragraph><paragraph>Bar2</paragraph><paragraph>[]</paragraph>',
        '<p>Foo1</p><p>Bar2</p><p>[]</p>'
      );

      editor.execute('undo');

      expectOutput('<paragraph>Foo1</paragraph><paragraph>Bar2[]</paragraph>', '<p>Foo1</p><p>Bar2{}</p>');

      editor.execute('undo');

      expectOutput('<paragraph>Foo1</paragraph><paragraph>Bar[]</paragraph>', '<p>Foo1</p><p>Bar{}</p>');
    ...
    it('resets the buffer when attribute changes', () => {
      ...
      expectOutput(
        '<paragraph>Foo <$text bold="true">B[]</$text> Bar</paragraph>',
