...
describe('Bug ckeditor5-typing#100', () => {
  ...
  beforeEach(() => {
    ...
    return ClassicTestEditor.create(domElement, { plugins: [Input, Paragraph, Bold, Italic, LinkEditing] }).then(
      newEditor => {
        editor = newEditor;
        model = editor.model;
  ...
  it('should handle mutations switching inner and outer node when adding new text node after', () => {
    setModelData(model, '<paragraph>' + '<$text italic="true" bold="true">' + 'text[]' + '</$text>' + '</paragraph>');
  ...
  it('should handle mutations switching inner and outer node - with text before', () => {
    setModelData(
      model,
      '<paragraph>' + 'xxx<$text italic="true" bold="true">' + 'text[]' + '</$text>' + '</paragraph>'
    );
  ...
  it('should handle mutations replacing node', () => {
    setModelData(model, '<paragraph>' + '<$text bold="true">' + 'text[]' + '</$text>' + '</paragraph>');
  ...
  it('should handle mutations inside attribute element', () => {
    setModelData(model, '<paragraph>' + '<$text bold="true">' + 'this is foo text[]' + '</$text>' + '</paragraph>');
    ...
    viewDocument.fire('mutations', [
      {
        ...
        node: paragraph,
        oldChildren: [strong],
        newChildren: [new ViewElement('strong', null, new ViewText('text')), new ViewElement('img')],
      },
    ]);
