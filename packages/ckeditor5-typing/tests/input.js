...
describe('Input feature', () => {
  ...
  beforeEach(() => {
    ...
    return ClassicTestEditor.create(domElement, {
      plugins: [Input, Paragraph, Bold, Italic, List, ShiftEnter, Link],
    }).then(newEditor => {
      // Mock image feature.
  ...
  describe('mutations handling', () => {
    ...
    it('should apply selection attributes to the inserted text', () => {
      ...
      expect(getModelData(model)).to.equal('<paragraph><$text bold="true" italic="true">x[]</$text></paragraph>');
    });
      ...
      viewDocument.fire('mutations', [
        {
          ...
          newChildren: [new ViewElement('strong', null, new ViewText('Foo')), new ViewText(' '), new ViewElement('br')],
          node: viewRoot.getChild(0),
        },
      ]);
    ...
    it('should handle mutations correctly if there was an &nbsp; in model already', () => {
      ...
      viewDocument.fire('mutations', [
        {
          type: 'children',
          oldChildren: Array.from(
            viewRoot
              .getChild(0)
              .getChild(0)
              .getChildren()
          ),
          newChildren: [new ViewElement('strong', null, new ViewText('Fx'))],
          node: viewRoot.getChild(0).getChild(0),
        },
      ]);
      ...
      viewDocument.fire('mutations', [
        {
          type: 'children',
          oldChildren: [...viewRoot.getChild(0).getChildren()],
          newChildren: [new ViewText('Foo'), new ViewContainerElement('placeholder'), new ViewText('f')],
          node: viewRoot.getChild(0),
        },
      ]);
