...
class MutationHandler {
  ...
  _handleContainerChildrenMutations(mutations, viewSelection) {
    ...
    const modelFromCurrentDom = this.editor.data
      .toModel(freshDomConverter.domToView(domMutationCommonAncestor))
      .getChild(0);
    ...
    const newText = modelFromDomChildren
      .map(item => (item.is('text') ? item.data : '@'))
      .join('')
      .replace(/\u00A0/g, ' ');
    const oldText = currentModelChildren
      .map(item => (item.is('text') ? item.data : '@'))
      .join('')
      .replace(/\u00A0/g, ' ');
